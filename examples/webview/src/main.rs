    ...
    pub fn new(window: &mut Window, cx: &mut App) -> Entity<Self> {
        let webview = cx.new(|cx| {
            ...
            let webview = {
                ...
                let window_handle = window.window_handle().expect("Pencere tanıtıcısı yok");
                builder.build_as_child(&window_handle).unwrap()
            };

            WebGorunumu::new(webview, window, cx)
        });
...
fn main() {
    // Required this for Windows to render the WebGorunumu.
