...
/// WASM uygulaması - varlıkları ihtiyaç üzerine indirir
            ...
            if !is_pending {
                ...
                spawn_local(async move {
                    match reqwest::get(&url).await {
                        Ok(response) => {
                            if response.status().is_success() {
                                match response.bytes().await {
                                    ...
                                    Err(e) => {
                                        log::warn!("Simge okunamadı {}: {}", path_clone, e);
                                    }
                                }
                            } else {
                                log::warn!(
                                    "Simge indirilemedi {}: HTTP {}",
                                    path_clone,
                                    response.status()
                                );
                            }
                        ...
                        Err(e) => {
                            log::warn!("Simge alınamadı {}: {}", path_clone, e);
                        }
                    }
