...
use kavis_ui_story::{ButtonStory, IconStory, StoryContainer};
...
/// Diğer panellere ortak işlevler eklemek için onları saran kapsayıcı panel belirtimi.
///
/// Örneğin:
///
...
impl StoryTiles {
    ...
    fn save_tiles(state: &YerlesimAlaniDurumu) -> Result<()> {
        println!("Save tiles...");
    ...
    fn set_scrollbar_show(dock_area: &mut YerlesimAlani, cx: &mut App) {
        match dock_area.center() {
            YerlesimOgesi::Tiles { view, .. } => {
                view.update(cx, |this, cx| {
                    this.set_scrollbar_show(Some(KaydirmaCubuguGosterimi::Always), cx);
                });
            }
            _ => {}
        }
    ...
    fn init_default_layout(
        ...
        YerlesimOgesi::tiles(panels, bounds, dock_area, window, cx)
    }
    ...
    pub fn new_local(cx: &mut App) -> Task<anyhow::Result<WindowHandle<KokGorunum>>> {
        ...
        cx.spawn(async move |cx| {
            ...
            let window = cx.open_window(options, |window, cx| {
                ...
                cx.new(|cx| KokGorunum::new(tiles_view, window, cx))
            })?;
...
impl Render for StoryTiles {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .child(BaslikCubugu::new().child(div().flex().items_center().child("Story Tiles")))
