...
fn init() {
    DilKaydi::singleton().register(
        "navi",
        &DilYapilandirmasi::new(
            "navi",
...
pub struct ExampleLspStore {
    ...
    diagnostics: Arc<RwLock<Vec<Tani>>>,
...
impl ExampleLspStore {
    ...
    fn diagnostics(&self) -> Vec<Tani> {
        let guard = self.diagnostics.read().unwrap();
        guard.clone()
    }

    fn update_diagnostics(&self, diagnostics: Vec<Tani>) {
...
impl Example {
    ...
    fn load_files(state: Entity<AgacDurumu>, path: PathBuf, cx: &mut App) {
        ...
        self._lint_task = cx.background_spawn(async move {
            ...
            for item in result.lines.iter() {
                let severity = match item.severity {
                    autocorrect::Severity::Error => TaniOnemi::Warning,
                    autocorrect::Severity::Warning => TaniOnemi::Hint,
                    autocorrect::Severity::Pass => TaniOnemi::Info,
                };
                ...
                diagnostics.push(Tani::new(start..end, message).with_severity(severity));
    ...
    fn on_action_open(&mut self, _: &Open, window: &mut Window, cx: &mut Context<Self>) {
        let path = cx.prompt_for_paths(PathPromptOptions {
            ...
            directories: true,
            multiple: false,
            prompt: Some("Secim a source file".into()),
        });
    ...
    fn render_line_number_button(
        ...
        Dugme::new("line-number")
            .when(self.line_number, |this| this.icon(SimgeAdi::Check))
            .label("Line Number")
            .ghost()
            .xsmall()
    ...
    fn render_soft_wrap_button(&self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        Dugme::new("soft-wrap")
            .ghost()
            .xsmall()
            .when(self.soft_wrap, |this| this.icon(SimgeAdi::Check))
            .label("Soft Wrap")
            .on_click(cx.listener(|this, _, window, cx| {
    ...
    fn render_show_whitespaces_button(
        ...
        Dugme::new("show-whitespace")
            .ghost()
            .xsmall()
            .when(self.show_whitespaces, |this| this.icon(SimgeAdi::Check))
    ...
    fn render_indent_guides_button(
        ...
        Dugme::new("indent-guides")
            .ghost()
            .xsmall()
            .when(self.indent_guides, |this| this.icon(SimgeAdi::Check))
            .label("Indent Guides")
            .on_click(cx.listener(|this, _, window, cx| {
    ...
    fn render_folding_button(&self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        Dugme::new("folding")
            .ghost()
            .xsmall()
            .when(self.folding, |this| this.icon(SimgeAdi::Check))
            .label("Folding")
            .on_click(cx.listener(|this, _, window, cx| {
...
fn main() {
    ...
    app.run(move |cx| {
        ...
        kavis_ui_story::create_new_window_with_size(
            "Editor",
