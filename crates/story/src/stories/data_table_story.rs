        ...
        Self {
            ...
            columns: vec![
                ...
                Column::new("name", "Ad")
                    ...
                    .fixed(ColumnFixed::Left)
                    .max_width(300.),
                Column::new("symbol", "Sembol")
                    .width(100.)
                    .fixed(ColumnFixed::Left)
                    .sortable(),
                Column::new("price", "Fiyat").sortable().text_right().p_0(),
                Column::new("change", "Değişim")
                    .sortable()
                    .text_right()
                    .p_0(),
                Column::new("change_percent", "Değişim%")
                    .sortable()
                    .text_right()
                    .p_0(),
                Column::new("volume", "Hacim").p_0(),
                Column::new("turnover", "Ciro").p_0(),
                Column::new("market_cap", "Piyasa Değeri").p_0(),
                Column::new("ttm", "TTM").p_0(),
                Column::new("five_mins_ranking", "5dk Sıralama")
                    .text_right()
                    .p_0(),
                Column::new("th60_days_ranking", "60g Sıralama"),
                Column::new("year_change_percent", "Yıllık Değişim%"),
                Column::new("bid", "Alış").text_right().p_0(),
                Column::new("bid_volume", "Alış Hacmi").text_right().p_0(),
                Column::new("ask", "Satış").text_right().p_0(),
                Column::new("ask_volume", "Satış Hacmi").text_right().p_0(),
                Column::new("open", "Açılış").text_right().p_0(),
                Column::new("prev_close", "Önceki Kapanış")
                    .text_right()
                    .p_0(),
                Column::new("high", "Yüksek").text_right().p_0(),
                Column::new("low", "Düşük").text_right().p_0(),
                Column::new("turnover_rate", "Ciro Oranı"),
                Column::new("rise_rate", "Yükseliş Oranı"),
                Column::new("amplitude", "Genlik"),
                Column::new("pe_status", "P/E"),
                Column::new("pb_status", "P/B"),
                Column::new("volume_ratio", "Hacim Oranı")
                    .text_right()
                    .p_0(),
                Column::new("bid_ask_ratio", "Alış Satış Oranı")
                    .text_right()
                    .p_0(),
                Column::new("latest_pre_close", "Son Ön Kapanış"),
                Column::new("latest_post_close", "Son Sonrası Kapanış"),
                Column::new("pre_market_cap", "Ön Piyasa Değeri"),
                Column::new("pre_market_percent", "Ön Piyasa%"),
                Column::new("pre_market_change", "Ön Piyasa Değişimi"),
                Column::new("post_market_cap", "Son Piyasa Değeri"),
                Column::new("post_market_percent", "Son Piyasa%"),
                Column::new("post_market_change", "Son Piyasa Değişimi"),
                Column::new("float_cap", "Dolaşımdaki Değer"),
                Column::new("shares", "Hisseler"),
                Column::new("shares_float", "Dolaşımdaki Hisseler"),
                Column::new("day_5_ranking", "5g Sıralama"),
                Column::new("day_10_ranking", "10g Sıralama"),
                Column::new("day_30_ranking", "30g Sıralama"),
                Column::new("day_120_ranking", "120g Sıralama"),
                Column::new("day_250_ranking", "250g Sıralama"),
            ],
...
impl TabloTemsilcisi for StockTableDelegate {
    ...
    fn column(&self, col_ix: usize, _cx: &App) -> Column {
        if let Some(col) = self.columns.get(col_ix) {
            ...
            Column::new(format!("extra_{n}"), format!("Sütun {n}"))
        }
    }
    ...
    fn group_headers(&self, cx: &App) -> Option<Vec<Vec<ColumnGroup>>> {
        ...
        Some(vec![
            vec![
                ColumnGroup {
                    label: "Hisse Bilgisi".into(),
                    span: 4,
                },
                ColumnGroup {
                    label: "Fiyat ve Değişim".into(),
                    span: 3,
                },
            ],
            vec![
                ...
                ColumnGroup {
                    label: "Sıralama ve İstatistikler".into(),
                    span: 14,
                },
                ColumnGroup {
    ...
    fn render_tr(
        ...
        div()
            ...
            .on_click(cx.listener(move |table, ev: &ClickEvent, _window, cx| {
                println!(
                    "Satıra ikincil seçimle tıkladınız: {}",
    ...
    /// NOT: Performans ölçümleri
    ///
    /// son çizim toplam 561 hücre: 232.745µs, ortalama: 414ns
    /// Kare süresi: 8.825083ms
    ///
    /// Bu, tüm tablo hücrelerini çizmenin 232.745µs sürdüğü anlamına gelir. 232.745µs / 8.82ms = kare süresinin %2.6'sı.
    ///
    /// td çizimini iyileştirirsek tüm tablo hücrelerini çizme süresini azaltabiliriz.
    fn render_td(
        &mut self,
        row_ix: usize,
        col_ix: usize,
    ...
    fn load_more(&mut self, _: &mut Window, cx: &mut Context<TabloDurumu<Self>>) {
        if !self.lazy_load {
            return;
        }
...
impl DataTableStory {
    ...
    fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let table = cx.new(|cx| TabloDurumu::new(delegate, window, cx));
    ...
    fn on_table_event(
        &mut self,
        _: &Entity<TabloDurumu<StockTableDelegate>>,
        event: &TabloOlayi,
        _window: &mut Window,
        _cx: &mut Context<Self>,
    ) {
        match event {
            TabloOlayi::ColumnWidthsChanged(col_widths) => {
                println!("Sütun genişlikleri değişti: {:?}", col_widths)
            }
            TabloOlayi::SelectColumn(ix) => println!("Sütun seçildi: {}", ix),
            TabloOlayi::SelectCell(row_ix, col_ix) => {
                println!("Hücre seçildi: satır={}, sütun={}", row_ix, col_ix)
            }
            TabloOlayi::DoubleClickedCell(row_ix, col_ix) => {
                println!("Hücreye çift tıklandı: satır={}, sütun={}", row_ix, col_ix)
            }
            TabloOlayi::DoubleClickedRow(ix) => println!("Satıra çift tıklandı: {}", ix),
            TabloOlayi::SelectRow(ix) => println!("Satır seçildi: {}", ix),
            TabloOlayi::MoveColumn(origin_idx, target_idx) => {
                println!("Sütun indeksi taşındı: {} -> {}", origin_idx, target_idx);
            }
            TabloOlayi::RightClickedRow(ix) => println!("Satıra sağ tıklandı: {:?}", ix),
            TabloOlayi::RightClickedCell(row_ix, col_ix) => {
                println!("Hücreye sağ tıklandı: satır={}, sütun={}", row_ix, col_ix)
            }
            TabloOlayi::ClearSelection => {
                println!("Seçim temizlendi");
            }
        }
    }
        ...
        match self.write_csv(cx) {
            Ok(csv_content) => {
                let Some(path) = dirs::download_dir() else {
                    eprintln!("İndirilenler dizini alınamadı");
                    return;
                };
                ...
                cx.spawn_in(window, async move |_, _| {
                    if let Some(path) = receiver.await.ok().into_iter().flatten().flatten().next() {
                        match std::fs::write(&path, csv_content) {
                            Ok(_) => {
                                println!("CSV başarıyla dışa aktarıldı: {:?}", path);
                            }
                            Err(e) => {
                                eprintln!("CSV dosyası kaydedilemedi: {}", e);
                            }
                        }
                    } else {
                        println!("CSV dışa aktarma kullanıcı tarafından iptal edildi");
                    };
                })
                .detach();
            }
            Err(e) => {
                eprintln!("CSV dışa aktarılamadı: {}", e);
            }
        }
    }
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        OnayKutusu::new("refresh-data")
                            .label("Veriyi Yenile")
                            .selected(self.refresh_data)
                            .on_click(cx.listener(Self::toggle_refresh_data)),
                    )
                    .child(
                        OnayKutusu::new("group-headers")
                            .label("Grup Başlıkları")
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        Dugme::new("select-cell-10-7")
                            .outline()
                            .small()
                            .child("Hücre Seç (10, 7)")
                            .on_click(cx.listener(|this, _, _, cx| {
                                this.table.update(cx, |table, cx| {
                    ...
                    .child(
                        Dugme::new("clear-selection")
                            .outline()
                            .small()
                            .child("Seçimi Temizle")
                            .on_click(cx.listener(|this, _, _, cx| {
                                this.table.update(cx, |table, cx| {
                    ...
                    h_flex()
                        ...
                        .child(
                            h_flex()
                                ...
                                .child(
                                    OnayKutusu::new("lazy-load")
                                        .label("Tembel Yükleme")
                        ...
                        .child(
                            h_flex()
                                .gap_2()
                                .child(format!("Toplam Satır: {}", rows_count))
                                .child(format!("Görünen Satırlar: {:?}", delegate.visible_rows))
                                .child(format!("Görünen Sütunlar: {:?}", delegate.visible_cols))
                                .when_some(table.selected_cell(), |this, (row, col)| {
                                    this.child(format!("Seçili Hücre: ({}, {})", row, col))
                                })
                                .when(delegate.eof, |this| this.child("Tüm veriler yüklendi.")),
                        ),
                ),
            )
            .child(
                VeriTablosu::new(&self.table)
                    .with_size(self.size)
                    .stripe(self.stripe),
            )
    }
