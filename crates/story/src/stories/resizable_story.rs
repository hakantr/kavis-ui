...
pub struct ResizableStory {
    ...
    show_left: bool,
    use_flex_none: bool,
    programmatic_state: Entity<YenidenBoyutlandirilabilirDurum>,
}
...
impl ResizableStory {
    ...
    fn new(_: &mut Window, cx: &mut App) -> Self {
        Self {
            ...
            show_left: true,
            use_flex_none: true,
            programmatic_state: cx.new(|_| YenidenBoyutlandirilabilirDurum::default()),
        }
    }
}
...
impl Render for ResizableStory {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        v_flex()
                    ...
                    .child(
                        v_resizable("resizable-1")
                            .on_resize(|state, _, cx| {
                                println!("Yeniden boyutlandırıldı: {:?}", state.read(cx).sizes());
                            })
                            .child(
            ...
            // Demonstrates `.flex_none()`. Gecis the left panel's
            ...
            // `YenidenBoyutlandirilabilirDurum::resize_panel(ix, size, window, cx)`. Buttons
            // mutate the shared state; subscribers (none here) would observe
            // a `YenidenBoyutlandirilabilirPanelOlayi::Resized` just like a drag-finish.
            .child(
                v_flex()
                    .gap_2()
                    .child(
