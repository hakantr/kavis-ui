...
use kavis_ui::{
    ...
    h_flex,
    label::Etiket,
    list::{Liste, ListeDurumu, ListeOgesi, ListeOlayi, ListeTemsilcisi},
    v_flex,
};
...
impl CompanyListItem {
    pub fn new(id: impl Into<ElementId>, company: Rc<Company>, selected: bool) -> Self {
        CompanyListItem {
            company,
            base: ListeOgesi::new(id).selected(selected),
            selected,
        }
    }
}
                    ...
                    .child(
                        h_flex().gap_2().child(
                            v_flex()
                                ...
                                .overflow_x_hidden()
                                .flex_nowrap()
                                .child(Etiket::new(self.company.name.clone()).whitespace_nowrap()),
                        ),
                    )
                    .child(
                        h_flex()
...
impl ListeTemsilcisi for CompanyListDelegate {
    ...
    fn confirm(
        ...
        println!("İkincil seçimle onaylandı: {}", secondary);
    ...
    fn set_right_clicked_index(
        ...
        println!("sağ tıklanan indeks: {:?}", ix);
    }
    ...
    fn render_section_header(
        ...
        Some(
            h_flex()
                ...
                .child(Simge::new(SimgeAdi::Folder))
...
pub struct ListStory {
    ...
    company_list: Entity<ListeDurumu<CompanyListDelegate>>,
...
impl ListStory {
    ...
    fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let company_list = cx.new(|cx| ListeDurumu::new(delegate, window, cx).searchable(true));

        let _subscriptions =
            vec![
                cx.subscribe(&company_list, |_, _, ev: &ListeOlayi, _| match ev {
                    ListeOlayi::Secim(ix) => {
                        println!("Listede seçildi: {:?}", ix);
                    }
                    ListeOlayi::Confirm(ix) => {
                        println!("Listede onaylandı: {:?}", ix);
                    }
                    ListeOlayi::Cancel => {
                        println!("Liste iptal edildi");
                    }
                }),
            ];
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(
                        Dugme::new("scroll-selected")
                            .outline()
                            .child("Seçili öğeye kaydır")
                            .small()
                            .on_click(cx.listener(|this, _, window, cx| {
                                this.company_list.update(cx, |list, cx| {
                    ...
                    .child(
                        Dugme::new("scroll-to-item")
                            .outline()
                            .child("(5, 1) konumuna kaydır")
                            .small()
                            .on_click(cx.listener(|this, _, window, cx| {
                                this.company_list.update(cx, |list, cx| {
                                    list.scroll_to_item(
