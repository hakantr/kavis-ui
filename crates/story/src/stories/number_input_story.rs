...
use kavis_ui::{
    ...
    input::{AdimAksiyonu, GirdiDurumu, GirdiOlayi, MaskeDeseni, SayiGirdisi, SayiGirdisiOlayi},
    v_flex,
};
...
pub struct NumberInputStory {
    ...
    number_input1: Entity<GirdiDurumu>,
    number_input2: Entity<GirdiDurumu>,
    number_input2_value: u64,
    number_input3: Entity<GirdiDurumu>,
    number_input3_value: f64,
    number_input4: Entity<GirdiDurumu>,
...
impl NumberInputStory {
    ...
    fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let number_input1 = cx.new(|cx| {
            GirdiDurumu::new(window, cx)
                .placeholder("Normal tam sayı")
                .default_value(number_input1_value.to_string())
        });

        let number_input2 = cx.new(|cx| {
            GirdiDurumu::new(window, cx)
                .placeholder("Boyutsuz tam sayı")
                .pattern(Regex::new(r"^\d+$").unwrap())
        });

        let number_input3 = cx.new(|cx| {
            GirdiDurumu::new(window, cx)
                .placeholder("Maske deseni")
                .mask_pattern(MaskeDeseni::localized_number(Some(2)))
        });
    ...
    fn on_number_input_event(
        &mut self,
        this: &Entity<GirdiDurumu>,
        event: &SayiGirdisiOlayi,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        match event {
            SayiGirdisiOlayi::Step(step_action) => match step_action {
                AdimAksiyonu::Decrement => {
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        v_flex()
            ...
            .child(
                section("Normal Size")
                    .max_w(px(200.))
                    .child(SayiGirdisi::new(&self.number_input1)),
            )
            .child(
                section("Disabled")
                    .max_w(px(200.))
                    .child(SayiGirdisi::new(&self.disabled_input).disabled(true)),
            )
            .child(
                section("Small Size with suffix").max_w(px(200.)).child(
                    SayiGirdisi::new(&self.number_input2)
                        .small()
            ...
            .child(
                section("With mask pattern")
                    .max_w(px(200.))
                    .child(SayiGirdisi::new(&self.number_input3)),
            )
            .child(
                section("Without appearance").max_w(px(200.)).child(
                    SayiGirdisi::new(&self.number_input4)
                        .appearance(false)
