...
#[cfg(any(feature = "inspector", debug_assertions))]
pub use inspector::*;
pub use kavis_ui_macros::{Aksiyon, OgeyeDonus, simge_adli};
    ...
    ($namespace:path, [ $( $(#[$attr:meta])* $name:ident),* $(,)? ]) => {
        $(
            ...
            impl $name {
                ...
                fn __autogenerated() {
                    #[doc(hidden)]
                    fn __kavis_ui_action_builder() -> $crate::ham_gpui::MacroActionData {
                        $crate::ham_gpui::MacroActionData {
                            name: <$name as $crate::Action>::name_for_type(),
                            type_id: ::std::any::TypeId::of::<$name>(),
                            build: <$name as $crate::Action>::build,
                            json_schema: <$name as $crate::Action>::action_json_schema,
                            deprecated_aliases: <$name as $crate::Action>::deprecated_aliases(),
                            deprecation_message: <$name as $crate::Action>::deprecation_message(),
                            documentation: <$name as $crate::Action>::documentation(),
                        }
                    }

                    $crate::ham_gpui::private::inventory::submit! {
                        $crate::ham_gpui::MacroActionBuilder(__kavis_ui_action_builder)
                    }
                }
            }
            ...
            impl $crate::Action for $name {
                ...
                fn build(
                    _value: $crate::ham_gpui::private::serde_json::Value,
                ) -> $crate::Result<Box<dyn $crate::Action>> {
                    Ok(Box::new(Self))
                }
            }
        )*
