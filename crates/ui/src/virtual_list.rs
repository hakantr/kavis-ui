//! Farklı boyutlara sahip çok sayıda satır veya sütunu çizmek için sanal liste.
//!
//! > NOT: Her sütun genişliği veya satır yüksekliği sağlanmalıdır.
//!
...
//!
//! Farklı satır yüksekliklerine sahip tablolar gibi daha karmaşık yerleşimler için kullanışlıdır.
use std::{
    cell::RefCell,
    cmp,
    ops::{Deref, Range},
...
/// [`SanalListe`] için kaydırma işleyicisi.
///
/// Bakınız ayrıca [`ScrollHandle`].
#[derive(Clone)]
pub struct SanalListeKaydirmaTutamaci {
    state: Rc<RefCell<VirtualListScrollHandleState>>,
    base_handle: ScrollHandle,
}

impl From<ScrollHandle> for SanalListeKaydirmaTutamaci {
    fn from(handle: ScrollHandle) -> Self {
        let mut this = SanalListeKaydirmaTutamaci::new();
        this.base_handle = handle;
        this
    }
}

impl AsRef<ScrollHandle> for SanalListeKaydirmaTutamaci {
    fn as_ref(&self) -> &ScrollHandle {
        &self.base_handle
    }
}

impl KaydirmaCubuguTutamaci for SanalListeKaydirmaTutamaci {
...
impl Deref for SanalListeKaydirmaTutamaci {
...
impl SanalListeKaydirmaTutamaci {
    /// Yeni bir SanalListeKaydirmaTutamaci oluşturur.
    pub fn new() -> Self {
        SanalListeKaydirmaTutamaci {
    ...
    /// Temel kaydırma işleyicisini döndürür.
    ...
    /// Verilen indeksteki öğeye, ek öğe ofsetiyle birlikte kaydırır.
...
/// Dikey yönde bir [`SanalListe`] oluşturur.
///
/// GPUI'daki `uniform_list` benzeridir, ancak iki ekseni de destekler.
///
/// `item_sizes` her öğenin boyutunu içerir. Dikey listede yalnızca `height`
/// kullanılır; `width` yok sayılır ve SanalListe genişliği ilk öğeden ölçer.
///
/// Bakınız: [`h_virtual_list`].
#[inline]
...
/// Yatay yönde bir [`SanalListe`] oluşturur.
///
/// `item_sizes` her öğenin boyutunu içerir. Yatay listede yalnızca `width`
/// kullanılır; `height` yok sayılır ve SanalListe yüksekliği ilk öğeden ölçer.
///
/// Bakınız: [`v_virtual_list`].
#[inline]
    ...
    V: Render,
{
    ...
    let scroll_handle = SanalListeKaydirmaTutamaci::new();
...
/// Farklı boyutlara sahip çok sayıda öğeyi çizmek için SanalListe bileşeni.
pub struct SanalListe {
    id: ElementId,
    axis: Axis,
    base: Stateful<Div>,
...
impl SanalListe {
    pub fn track_scroll(mut self, scroll_handle: &SanalListeKaydirmaTutamaci) -> Self {
        self.base = self.base.track_scroll(&scroll_handle);
        self.scroll_handle = scroll_handle.clone();
        self
    }

    /// Liste için boyutlandırma davranışını ayarlar.
    pub fn with_sizing_behavior(mut self, behavior: ListSizingBehavior) -> Self {
        self.sizing_behavior = behavior;
        self
    }

    /// Tablo kullanımı için özel kaydırma işleyicisi belirtir.
    ///
    /// Tablo özeldir; `scroll_handle` sanal liste olmayan tablo başlığına dayanır.
    pub(crate) fn with_scroll_handle(mut self, scroll_handle: &SanalListeKaydirmaTutamaci) -> Self {
    ...
    /// Ref den: https://github.com/zed-industries/zed/blob/83f9f9d9e3f5914392cab9a09e3472711a1d7b38/crates/gpui/src/öğeler/uniform_list.rs#L660
    fn measure_item(
        &self,
...
/// Frame durum kullanılır ile [VirtualItem].
pub struct SanalListeCerceveDurumu {
    /// Çizilecek görünür öğeler.
...
impl IntoElement for SanalListe {
    type Element = Self;
...
impl Element for SanalListe {
    type RequestLayoutState = SanalListeCerceveDurumu;
