...
use super::{
    ClosePanel, Panel, PanelControl, PanelEvent, PanelState, PanelStyle, PanelView, StackPanel,
    ToggleZoom, YerlesimAlani, YerlesimKonumu,
};
...
pub(crate) struct DragPanel {
    ...
    pub(crate) tab_panel: Entity<SekmePaneli>,
}

impl DragPanel {
    pub(crate) fn new(panel: Arc<dyn PanelView>, tab_panel: Entity<SekmePaneli>) -> Self {
...
pub struct SekmePaneli {
    focus_handle: FocusHandle,
    dock_area: WeakEntity<YerlesimAlani>,
    /// `stack_panel` None ise paneller bölünemez veya taşınamaz.
    stack_panel: Option<WeakEntity<StackPanel>>,
    pub(crate) panels: Vec<Arc<dyn PanelView>>,
    pub(crate) active_ix: usize,
    /// True ise panelin kapatılabilirliği etkin panelin kapatılabilirliğini izler;
    /// aksi halde bu SekmePaneli kapatılamaz.
    ///
    /// Yerlesim içinde son SekmePaneli'nin kapatılamamasını sağlamak için kullanılır;
    /// bakınız [`super::Yerlesim::new`].
    ...
    zoomed: bool,
    collapsed: bool,
    /// Sürükleme sırasında panelin bölüneceği yerleşimi tutar.
    will_split_placement: Option<Placement>,
    /// SekmePaneli'nin Tiles içinde kullanılıp kullanılmadığı.
    in_tiles: bool,
}

impl Panel for SekmePaneli {
    ...
    fn title(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        self.active_panel(cx)
            ...
            .unwrap_or("Empty Sekme".into_any_element())
    }
...
impl SekmePaneli {
    ...
    /// Mark SekmePaneli olarak olma kullanılır içinde Tiles.
    ...
    /// geçerli active_panel Görünüm döndürür.
    ...
    /// Sekmelerin sonuna bir panel ekler.
    pub fn add_panel(
        &mut self,
    ...
    fn add_panel_with_active(
        ...
        assert_ne!(
            panel.panel_name(cx),
            "StackPanel",
            "can not allows add `StackPanel` to `SekmePaneli`"
        );
    ...
    /// Paneli bölme yerleşimiyle eklemeyi dener.
    pub fn add_panel_at(
        &mut self,
        panel: Arc<dyn PanelView>,
        placement: Placement,
    ...
    /// Bir paneli sekme panelinden kaldırır.
    pub fn remove_panel(
        &mut self,
        panel: Arc<dyn PanelView>,
        window: &mut Window,
    ...
    /// Panel kalmadıysa kendisini üst StackPanel içinden kaldırır.
    ...
    /// Kendisi veya üst öğesi yalnızca son panele sahipse true döndürür.
    ...
    /// Tüm görünür panelleri döndürür.
    ...
    /// Sekme paneli sürüklenebilirse true döndürür.
    ///
    /// Örneğin üst öğe ve kendisi yalnızca bir panele sahipse sürüklenemez.
    fn draggable(&self, cx: &App) -> bool {
        !self.is_locked(cx) && !self.is_last_panel(cx)
    }

    /// Sekme paneli üzerine bırakma yapılabiliyorsa true döndürür.
    ///
    /// Örneğin sekme paneli kilitliyse üzerine bırakma yapılamaz.
    ...
    fn render_toolbar(
        ...
        h_flex()
            ...
            .map(|this| {
                let value = if zoomed {
                    Some(("zoom-out", SimgeAdi::Minimize, t!("Yerlesim.Zoom Out")))
                } else if zoomable_toolbar_visible {
                    Some(("zoom-in", SimgeAdi::Maximize, t!("Yerlesim.Zoom In")))
                } else {
                    None
                };
                        ...
                        move |menu, window, cx| {
                            view.update(cx, |this, cx| {
                                this.dropdown_menu(menu, window, cx)
                                    ...
                                    .when(closable, |this| {
                                        this.separator()
                                            .menu(t!("Yerlesim.Close"), Box::new(ClosePanel))
                                    })
                            })
                        }
                    })
    ...
    fn render_dock_toggle_button(
        ...
        // Check if current SekmePaneli's entity_id matches the one stored in YerlesimAlani for this placement
        if !match placement {
            YerlesimKonumu::Left => {
        ...
        let icon = match placement {
            ...
            YerlesimKonumu::Bottom => {
                if is_open {
                    SimgeAdi::PanelBottom
                } else {
                    SimgeAdi::PanelBottomOpen
                }
            }
            YerlesimKonumu::Center => unreachable!(),
        };
        ...
        Some(
            Dugme::new(SharedString::from(format!("toggle-dock:{:?}", placement)))
                .icon(icon)
                .xsmall()
                .ghost()
                .tab_stop(false)
                .tooltip(match is_open {
                    true => t!("Yerlesim.Collapse"),
                    false => t!("Yerlesim.Expand"),
                })
    ...
    fn render_title_bar(
        ...
        let left_dock_button = self.render_dock_toggle_button(YerlesimKonumu::Left, window, cx);
        let bottom_dock_button = self.render_dock_toggle_button(YerlesimKonumu::Bottom, window, cx);
        let right_dock_button = self.render_dock_toggle_button(YerlesimKonumu::Right, window, cx);
                        ...
                        .on_click(cx.listener({
                            ...
                            move |view, _, window, cx| {
                                ...
                                if is_bottom_dock && is_collapsed {
                                    _ = dock_area.update(cx, |dock_area, cx| {
                                        dock_area.toggle_dock(YerlesimKonumu::Bottom, window, cx);
                                    });
                                }
                            }
                        }))
    ...
    /// Geçerli fare konumuna göre bölme yönünü hesaplar.
    ...
    /// Bir panel sürüklenirken bırakma olayını işler.
    ///
    /// - `active` - true olduğunda panel bırakmadan sonra etkin olur.
    fn on_drop(
        &mut self,
        drag: &DragPanel,
        ix: Option<usize>,
        ...
        // Cannot update ui::dock::tab_panel::SekmePaneli while it is already being updated
        if is_same_tab {
    ...
    /// Paneli bölme yerleşimiyle ekler.
    fn split_panel(
        &self,
        panel: Arc<dyn PanelView>,
        placement: Placement,
        ...
        let dock_area = self.dock_area.clone();
        // wrap the panel in a SekmePaneli
        ...
        if parent_axis.is_vertical() && placement.is_vertical() {
            ...
            // 3. Add the new SekmePaneli to the new StackPanel at the correct index
    ...
    fn on_action_close_panel(
        ...
        // Remove self from the parent YerlesimAlani.
