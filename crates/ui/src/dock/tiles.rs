...
/// TileItem, Tiles görünümüne eklenebilen taşınabilir ve yeniden boyutlandırılabilir paneldir.
#[derive(Clone)]
pub struct TileItem {
    id: EntityId,
...
/// Tiles, birden çok panel içerebilen bir tuvaldir; her panel sürüklenebilir ve yeniden boyutlandırılabilir.
pub struct Tiles {
...
impl Tiles {
    ...
    /// Sürüklenen panel için manyetik yapışma konumunu hesaplar.
    ...
    /// Panel başlangıç noktasına sınır kısıtlarını uygular.
    ...
    pub fn add_item(
        ...
        let Ok(tab_panel) = item.panel.view().downcast::<SekmePaneli>() else {
            panic!("only allows to add SekmePaneli type")
        };
    ...
    /// Paneli hedef indeks için öne getirir; başarılıysa yeni panel kimliğini döndürür.
    fn bring_to_front(
        &mut self,
    ...
    /// Produce bir vector AnyElement representing three possible yeniden boyutlandırma handles
    fn render_resize_handles(
        ...
        elements.push(
            div()
                .child(
                    Simge::new(SimgeAdi::ResizeCorner)
                        .size_3()
                        .absolute()
                        .right(px(1.))
                        .bottom(px(1.))
    ...
    /// Verilen panel öğesi için sürükleme çubuğu öğesini üretir.
    fn render_drag_bar(
        &mut self,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    /// İşler fare up olay için finalize sürükleme veya yeniden boyutlandırma işlemler
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .bottom_0()
                    .child(
                        KaydirmaCubugu::new(&self.scroll_handle)
