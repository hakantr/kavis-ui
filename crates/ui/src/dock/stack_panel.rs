...
use crate::{
    ...
    h_flex,
    resizable::{
        PANEL_MIN_SIZE, YenidenBoyutlandirilabilirDurum, YenidenBoyutlandirilabilirPanelDurumu,
        YenidenBoyutlandirilabilirPanelGrubu, YenidenBoyutlandirilabilirPanelOlayi,
        resizable_panel,
    },
};
...
impl StackPanel {
    pub fn new(axis: Axis, _: &mut Window, cx: &mut Context<Self>) -> Self {
        let state = cx.new(|_| YenidenBoyutlandirilabilirDurum::default());

        let _subscriptions = vec![
            // Bubble up the resize event.
            cx.subscribe(
                &state,
                |_, _, _: &YenidenBoyutlandirilabilirPanelOlayi, cx| {
    ...
    /// Üst düzey stack panel köktür ve üst öğesi yoktur.
    ...
    fn assert_panel_is_valid(&self, panel: &Arc<dyn PanelView>) {
        assert!(
            panel.view().downcast::<SekmePaneli>().is_ok()
                || panel.view().downcast::<StackPanel>().is_ok(),
            "Panel must be a `SekmePaneli` or `StackPanel`"
        );
    }

    /// Stack sonuna bir panel ekler.
    ///
    /// `size` `None` ise panel, stack içindeki tüm panellerin ortalama boyutuna ayarlanır.
    ///
    /// `panel` bir [`SekmePaneli`] veya [`StackPanel`] olmalıdır.
    pub fn add_panel(
        &mut self,
    ...
    /// [`Placement`] konumuna bir panel ekler.
    ///
    /// `panel` bir [`SekmePaneli`] veya [`StackPanel`] olmalıdır.
    pub fn add_panel_at(
        &mut self,
    ...
    /// Belirtilen indekse bir panel ekler.
    ///
    /// `panel` bir [`SekmePaneli`] veya [`StackPanel`] olmalıdır.
    ...
    /// Belirtilen indeksten önce bir panel ekler.
    ///
    /// `panel` bir [`SekmePaneli`] veya [`StackPanel`] olmalıdır.
    ...
    /// Belirtilen indeksten sonra bir panel ekler.
    ///
    /// `panel` bir [`SekmePaneli`] veya [`StackPanel`] olmalıdır.
    ...
    fn insert_panel(
        ...
        window.defer(cx, {
            ...
            move |window, cx| {
                // If the panel is a SekmePaneli, set its parent to this.
                if let Ok(tab_panel) = panel.view().downcast::<SekmePaneli>() {
    ...
    /// Paneli stack içinden kaldırır.
    ///
    /// `ix` bulunamazsa işlem yapmaz.
    pub fn remove_panel(
        &mut self,
        panel: Arc<dyn PanelView>,
        window: &mut Window,
    ...
    /// Aynı indeksteki eski paneli yeni panelle değiştirir.
    pub(super) fn replace_panel(
        ...
        if let Some(ix) = self.index_of_panel(old_panel.clone()) {
            ...
            let panel_state = YenidenBoyutlandirilabilirPanelDurumu::default();
            self.state.update(cx, |state, cx| {
    ...
    /// Alt öğeler boşsa kendisini üst öğe görünümünden kaldırır.
    ...
    /// Stack içindeki sol üst ilk sekme panelini bulur.
    pub(super) fn left_top_tab_panel(
        &self,
        check_parent: bool,
        cx: &App,
    ) -> Option<Entity<SekmePaneli>> {
        if check_parent {
        ...
        if let Some(view) = first_panel {
            if let Ok(tab_panel) = view.view().downcast::<SekmePaneli>() {
                Some(tab_panel)
    ...
    /// Stack içindeki sağ üst ilk sekme panelini bulur.
    ...
    /// Stack içindeki tüm panelleri kaldırır.
    ...
    /// Stack panel eksenini değiştirir.
...
impl Render for StackPanel {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        h_flex()
            ...
            .bg(cx.theme().tab_bar)
            .child(
                YenidenBoyutlandirilabilirPanelGrubu::new("stack-panel-group")
