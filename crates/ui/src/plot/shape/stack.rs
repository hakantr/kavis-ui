...
/// Alt ve üst değerleri olan yığılmış seri veri noktasını temsil eder.
#[derive(Clone, Debug)]
pub struct StackPoint<T> {
    /// Alt değer (taban çizgisi).
    pub y0: f32,
    /// Üst değer (tepe çizgisi).
    pub y1: f32,
    /// Özgün veriye referans.
    pub data: T,
}

/// Yığılmış bir seriyi temsil eder.
#[derive(Clone, Debug)]
...
impl<T: Clone> Stack<T> {
    ...
    /// Yığılacak anahtarları (serileri) ayarlar.
    ...
    /// Değer erişim fonksiyonunu ayarlar.
