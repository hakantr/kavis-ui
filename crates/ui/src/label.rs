...
/// Etiket içindeki metni vurgulamak için eşleşme tipini temsil eder.
#[derive(Clone)]
...
/// İsteğe bağlı ikincil metin, maskeleme ve vurgulama özelliklerine sahip metin etiketi öğesi.
#[derive(IntoElement)]
pub struct Etiket {
...
impl Etiket {
    /// Yeni bir etiket ile ana etiket oluşturur.
    ...
    /// ikincil metin için etiket, ayarlar.
    /// İkincil metin, etiket metninden sonra `muted` renkle gösterilir.
    pub fn secondary(mut self, secondary: impl Into<SharedString>) -> Self {
        self.secondary = Some(secondary.into());
        self
    }

    /// olup olmadığını için maske etiket metin ayarlar.
    pub fn masked(mut self, masked: bool) -> Self {
        self.masked = masked;
        self
    }

    /// için eşleşen metin için vurgulama içinde etiket ayarlar.
...
mod tests {
    ...
    fn test_highlight_ranges() {
        ...
        let label = Etiket::new("Hello World").highlights("WORLD");
        let result = label.highlight_ranges("Hello World".len());
        assert_eq!(result.len(), 1);
        assert_eq!(result[0], 6..11); // "World"

        // Multiple matches
        let label = Etiket::new("Hello Hello Hello").highlights("Hello");
        ...
        let label = Etiket::new("Hello World").highlights("xyz");
        let result = label.highlight_ranges("Hello World".len());
        assert_eq!(result, Vec::<Range<usize>>::new());

        let label = Etiket::new("Hello World").highlights("");
        let result = label.highlight_ranges("Hello World".len());
        assert_eq!(result, Vec::<Range<usize>>::new());
        ...
        let label = Etiket::new("Hello").secondary("World").highlights("llo");
        let total_length = "Hello World".len();
        let result = label.highlight_ranges(total_length);
        assert_eq!(result.len(), 3);
        ...
        let label = Etiket::new("Hello").secondary("World").highlights("World");
        let total_length = "Hello World".len();
        let result = label.highlight_ranges(total_length);
        assert_eq!(result.len(), 3);
        assert_eq!(result[0], 0..5); // Main text range
        ...
        let label = Etiket::new("Hello").secondary("World").highlights("o W");
        let total_length = "Hello World".len();
        let result = label.highlight_ranges(total_length);
        assert_eq!(result.len(), 3);
        assert_eq!(result[0], 0..5); // Main text range
        ...
        // Overlapping matches
        let label = Etiket::new("aaaa").highlights("aa");
        ...
        // Unicode text
        let label = Etiket::new("你好世界，Hello World").highlights("世界");
    ...
    fn test_highlight_ranges_prefix() {
        ...
        let label = Etiket::new("aaaa").highlights(HighlightsMatch::Prefix("aa".into()));
        let result = label.highlight_ranges("aaaa".len());
        assert_eq!(result.len(), 1);
        ...
        let label_full =
            Etiket::new("Hello Hello").highlights(HighlightsMatch::Full("Hello".into()));
        let result_full = label_full.highlight_ranges("Hello Hello".len());
        assert_eq!(result_full.len(), 2); // Both "Hello" matches

        let label_prefix =
            Etiket::new("Hello Hello").highlights(HighlightsMatch::Prefix("Hello".into()));
        ...
        let label =
            Etiket::new("Hello hello HELLO").highlights(HighlightsMatch::Prefix("hello".into()));
        let result = label.highlight_ranges("Hello hello HELLO".len());
        assert_eq!(result.len(), 1);
        assert_eq!(result[0], 0..5); // First "Hello" (case insensitive)

        // Test prefix with no match
        let label = Etiket::new("Hello World").highlights(HighlightsMatch::Prefix("xyz".into()));
        let result = label.highlight_ranges("Hello World".len());
        assert_eq!(result.len(), 0);

        // Test prefix with empty string
        let label = Etiket::new("Hello World").highlights(HighlightsMatch::Prefix("".into()));
        let result = label.highlight_ranges("Hello World".len());
        assert_eq!(result.len(), 0);

        // Test prefix with secondary text - match in main text
        let label = Etiket::new("Hello")
        ...
        let label = Etiket::new("你好世界你好").highlights(HighlightsMatch::Prefix("你好".into()));
        ...
        let label = Etiket::new("abababab").highlights(HighlightsMatch::Prefix("abab".into()));
        ...
        let label =
            Etiket::new("xyz Hello abc Hello").highlights(HighlightsMatch::Prefix("Hello".into()));
