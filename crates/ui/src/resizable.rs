...
/// Yatay yeniden boyutlandırma için bir [`YenidenBoyutlandirilabilirPanelGrubu`] oluşturur.
pub fn h_resizable(id: impl Into<ElementId>) -> YenidenBoyutlandirilabilirPanelGrubu {
    YenidenBoyutlandirilabilirPanelGrubu::new(id).axis(Axis::Horizontal)
}

/// Dikey yeniden boyutlandırma için bir [`YenidenBoyutlandirilabilirPanelGrubu`] oluşturur.
pub fn v_resizable(id: impl Into<ElementId>) -> YenidenBoyutlandirilabilirPanelGrubu {
    YenidenBoyutlandirilabilirPanelGrubu::new(id).axis(Axis::Vertical)
}

/// Bir [`YenidenBoyutlandirilabilirPanel`] oluşturur.
pub fn resizable_panel() -> YenidenBoyutlandirilabilirPanel {
    YenidenBoyutlandirilabilirPanel::new()
}

/// Bir [`YenidenBoyutlandirilabilirPanel`] için durum.
#[derive(Debug, Clone)]
pub struct YenidenBoyutlandirilabilirDurum {
    /// `axis`, kullanılan gerçek [`YenidenBoyutlandirilabilirPanelGrubu`] ekseniyle eşitlenir.
    axis: Axis,
    panels: Vec<YenidenBoyutlandirilabilirPanelDurumu>,
    sizes: Vec<Pixels>,
    pub(crate) resizing_panel_ix: Option<usize>,
    bounds: Bounds<Pixels>,
}

impl Default for YenidenBoyutlandirilabilirDurum {
    fn default() -> Self {
        Self {
...
impl YenidenBoyutlandirilabilirDurum {
    /// Panel boyutlarını döndürür.
    pub fn sizes(&self) -> &Vec<Pixels> {
        &self.sizes
    }

    /// `ix` konumundaki paneli programatik olarak `size` boyutuna getirir ve
    /// boşluğu kardeş öğeler arasında sürükleme mantığıyla yeniden dağıtır.
    ///
    /// Boyutlar panelin `size_range` değeri ve kapsayıcı boyutuyla sınırlandırılır.
    /// `YenidenBoyutlandirilabilirPanelOlayi::Resized` yayar; böylece aboneler
    /// (ör. tercih kalıcılığı) değişimi kullanıcı işleyiciyi sürüklemiş gibi görür.
    ///
    /// Aralık dışı indekslerde işlem yapılmaz. Son panelde boşluk önceki kardeş
    /// öğeden alınır; son panelin kendine ait işleyicisi yoktur.
    pub fn resize_panel(
        &mut self,
        ix: usize,
        size: Pixels,
    ...
    pub(crate) fn insert_panel(
        ...
        let panel_state = YenidenBoyutlandirilabilirPanelDurumu {
            size,
            ..Default::default()
        };
    ...
    pub(crate) fn sync_panels_count(
        ...
        if panels_count > self.panels.len() {
            ...
            self.panels
                .extend(vec![YenidenBoyutlandirilabilirPanelDurumu::default(); diff]);
    ...
    pub(crate) fn replace_panel(
        &mut self,
        panel_ix: usize,
        panel: YenidenBoyutlandirilabilirPanelDurumu,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(crate) fn done_resizing(&mut self, cx: &mut Context<Self>) {
        ...
        cx.emit(YenidenBoyutlandirilabilirPanelOlayi::Resized);
    }
    ...
    /// `ix` değerini sürükleme işleyicisi konumu kabul ederek paneli yeniden boyutlandırır.
    /// İşleyici `ix` ve `ix + 1` panellerinin arasında yer alır. Son panelin altında
    /// işleyici olmadığı için son panelde erken döner.
    ///
    /// Sürükleme etkileşimlerinin ve genel [`Self::resize_panel`] API'sinin
    /// arkasındaki yardımcı işlemdir.
    ...
    /// Panel boyutlarını kapsayıcı boyutuna göre ayarlar.
    ///
    /// Kapsayıcı boyutu değiştiğinde paneller önceki oranlarını korumalıdır.
...
impl EventEmitter<YenidenBoyutlandirilabilirPanelOlayi> for YenidenBoyutlandirilabilirDurum {}

#[derive(Debug, Clone, Default)]
pub(crate) struct YenidenBoyutlandirilabilirPanelDurumu {
