...
/// genişlik kaydırma çubuğu (THUMB_ACTIVE_INSET * 2 + THUMB_ACTIVE_WIDTH)
...
/// KaydirmaCubugu gösterir mod.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Hash, Default, JsonSchema)]
pub enum KaydirmaCubuguGosterimi {
    /// Kaydırma sırasında kaydırma çubuğunu gösterir; boşta kalınca söner.
    #[default]
    Scrolling,
    /// Gösterir kaydırma çubuğu üzerinde üzerine gelme.
    Hover,
    /// Always gösterir kaydırma çubuğu.
    Always,
}

impl KaydirmaCubuguGosterimi {
...
/// Kaydırma tutamaçları için ofset alıp ayarlayabilen trait.
pub trait KaydirmaCubuguTutamaci: 'static {
    /// geçerli ofset kaydırma işleyici döndürür.
    fn offset(&self) -> Point<Pixels>;
    /// ofset kaydırma işleyici ayarlar.
    fn set_offset(&self, offset: Point<Pixels>);
    /// tam boyut içerik, dahil dolgu.
    fn content_size(&self) -> Size<Pixels>;
    /// Kaydırma çubuğu tutamağı sürüklenmeye başlandığında çağrılır.
    fn start_drag(&self) {}
    /// Kaydırma çubuğu tutamağı sürükleme bittiğinde çağrılır.
    fn end_drag(&self) {}
}

impl KaydirmaCubuguTutamaci for ScrollHandle {
    fn offset(&self) -> Point<Pixels> {
        self.offset()
    }
...
impl KaydirmaCubuguTutamaci for UniformListScrollHandle {
    fn offset(&self) -> Point<Pixels> {
...
impl KaydirmaCubuguTutamaci for ListeDurumu {
    fn offset(&self) -> Point<Pixels> {
...
/// KaydirmaCubugu eksen.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum KaydirmaCubuguEkseni {
    /// Dikey kaydırma çubuğu.
    Vertical,
    /// Yatay kaydırma çubuğu.
    Horizontal,
    /// Gösterir ikisi de dikey ve yatay scrollbars.
    Both,
}

impl From<Axis> for KaydirmaCubuguEkseni {
...
impl KaydirmaCubuguEkseni {
    ...
    /// Kaydırma çubuğu ekseni hem dikey hem yataysa true döndürür.
    #[inline]
    pub fn is_both(&self) -> bool {
        matches!(self, Self::Both)
    }

    /// kaydırma çubuğu sahip dikey eksen ise true döndürür.
    #[inline]
    pub fn has_vertical(&self) -> bool {
        matches!(self, Self::Vertical | Self::Both)
    }

    /// kaydırma çubuğu sahip yatay eksen ise true döndürür.
    #[inline]
...
/// KaydirmaCubugu kontrol için kaydırma-alan veya bir uniform-liste.
pub struct KaydirmaCubugu {
    pub(crate) id: ElementId,
    axis: KaydirmaCubuguEkseni,
    scrollbar_show: Option<KaydirmaCubuguGosterimi>,
    scroll_handle: Rc<dyn KaydirmaCubuguTutamaci>,
    scroll_size: Option<Size<Pixels>>,
    /// Sürükleyerek kaydırma için saniye başına maksimum kare sayısı. Varsayılan 120 FPSdir.
    ///
    /// Bu için kullanılır sınır günceller rate kaydırma çubuğu olduğunda onu olur
    /// olma sürüklenmiş için bazı complex interactions için reducing CPU kullanım.
    max_fps: usize,
}
...
impl KaydirmaCubugu {
    ...
    ///
    /// Hem dikey hem yatay kaydırma çubukları bulunur.
    #[track_caller]
    pub fn new<H: KaydirmaCubuguTutamaci + Clone>(scroll_handle: &H) -> Self {
        let caller = Location::caller();
        Self {
            id: ElementId::CodeLocation(*caller),
            axis: KaydirmaCubuguEkseni::Both,
            scrollbar_show: None,
    ...
    /// Oluşturur ile yatay kaydırma çubuğu.
    #[track_caller]
    pub fn horizontal<H: KaydirmaCubuguTutamaci + Clone>(scroll_handle: &H) -> Self {
        Self::new(scroll_handle).axis(KaydirmaCubuguEkseni::Horizontal)
    }

    /// Oluşturur ile dikey kaydırma çubuğu.
    #[track_caller]
    pub fn vertical<H: KaydirmaCubuguTutamaci + Clone>(scroll_handle: &H) -> Self {
        Self::new(scroll_handle).axis(KaydirmaCubuguEkseni::Vertical)
    }

    /// Özel öğe id değerini ayarlar. Varsayılan [`Location::caller`] değeridir.
    ///
    /// NOT: Çoğu durumda kaydırma çubuğu için özel id ayarlamanız gerekmez.
    pub fn id(mut self, id: impl Into<ElementId>) -> Self {
        self.id = id.into();
        self
    }

    /// kaydırma çubuğu gösterir mod [`KaydirmaCubuguGosterimi`], ise değil ayarlar kullanım `cx.theme().scrollbar_show` ayarlar.
    pub fn scrollbar_show(mut self, scrollbar_show: KaydirmaCubuguGosterimi) -> Self {
        self.scrollbar_show = Some(scrollbar_show);
        self
    }

    /// İçerik alanı için özel kaydırma boyutunu ayarlar. Varsayılan None değeridir.
    ///
    /// Varsayılan olarak `scroll_handle` içindeki `content_size` değerine eşitlenir.
    pub fn scroll_size(mut self, scroll_size: Size<Pixels>) -> Self {
        self.scroll_size = Some(scroll_size);
        self
    }

    /// kaydırma çubuğu eksen ayarlar.
    pub fn axis(mut self, axis: impl Into<KaydirmaCubuguEkseni>) -> Self {
        self.axis = axis.into();
        self
    }

    /// Sürükleyerek kaydırma için saniye başına maksimum kare sayısını ayarlar. Varsayılan 120 FPSdir.
    ///
    /// siz sahip very yüksek CPU kullanım ise, consider reducing bu değer için improve performance.
    ///
    /// Available değerler: 30.120
    ...
    fn style_for_normal(&self, cx: &App) -> (Hsla, Hsla, Hsla, Pixels, Pixels, Pixels) {
        ...
        let (width, inset, radius) = match scrollbar_show {
            KaydirmaCubuguGosterimi::Scrolling => (THUMB_WIDTH, THUMB_INSET, THUMB_RADIUS),
...
impl IntoElement for KaydirmaCubugu {
    type Element = Self;
...
impl Element for KaydirmaCubugu {
