...
/// HistoryItem, geçmişteki tek bir değişikliği temsil eder.
/// Geçmişte kullanılabilmesi için Clone ve PartialEq uygulamalıdır.
pub trait HistoryItem: Clone + PartialEq {
    fn version(&self) -> usize;
    fn set_version(&mut self, version: usize);
}

/// Geçmiş, model değişikliklerini izlemek ve geri al/yinele işlemlerini desteklemek için kullanılır.
///
/// Bu artık girdide geri al/yinele işlemleri için kullanılır. Bunu ayrıca
/// your kendi models için korur track değişimler, için örnek için track sekme
/// geçmiş için önceki/sonraki features.
///
/// ## Kullanım Durumları
///
/// - Undo/yinele işlemler içinde girdi
/// - Tracking sekme geçmiş için önceki/sonraki features
#[derive(Debug)]
    ...
    I: HistoryItem,
{
    ...
    /// en yüksek sayı geri al steps için korur, varsayılan değer 1000 ayarlar.
    pub fn max_undos(mut self, max_undos: usize) -> Self {
        self.max_undos = max_undos;
        self
    }

    /// geçmiş olmak için unique, varsayılan değer false ayarlar.
    /// true ise geçmiş yalnızca benzersiz değişiklikleri tutar.
    pub fn unique(mut self) -> Self {
        self.unique = true;
        self
    }

    /// Değişiklikleri gruplamak için milisaniye cinsinden aralığı ayarlar. Varsayılan None değeridir.
    pub fn group_interval(mut self, group_interval: Duration) -> Self {
        self.group_interval = Some(group_interval);
        self
    }

    /// Değişiklik gruplamayı başlatır; `end_grouping` çağrılana kadar sürümün artırılmasını önler.
    pub fn start_grouping(&mut self) {
        self.grouping = true;
    }

    /// Değişiklik gruplamayı bitirir; sürümün yeniden artırılmasına izin verir.
    pub fn end_grouping(&mut self) {
        self.grouping = false;
    }

    /// Son değişiklik `GROUP_INTERVAL` milisaniyeden daha önce yapıldıysa sürüm sayısını artırır.
    ...
    /// geçerli version sayı döndürür.
    pub fn version(&self) -> usize {
        self.version
    }

    /// Push bir yeni değişir için geçmiş.
    ...
    /// geri al stack döndürür.
    pub fn undos(&self) -> &Vec<I> {
        &self.undos
    }

    /// yinele stack döndürür.
    pub fn redos(&self) -> &Vec<I> {
        &self.redos
    }

    /// Temizler geri al ve yinele stacks.
    pub fn clear(&mut self) {
        self.undos.clear();
        self.redos.clear();
    }

    /// Son değişikliği geri alır ve geri alınan değişiklikleri döndürür.
    ...
    /// Son geri alınmış değişikliği yineler ve yinelenen değişiklikleri döndürür.
