...
/// WASM üzerinde tree-sitter kullanılamadığı için Nokta yerine kullanılan yer tutucu tip.
...
#[cfg(target_family = "wasm")]
/// WASM üzerinde tree-sitter kullanılamadığı için InputEdit yerine kullanılan yer tutucu tip.
...
/// Bir iterator üzerinde satırlar bir `Rope`.
...
impl<'a> RopeLines<'a> {
    /// Yeni bir `RopeLines` iterator oluşturur.
...
/// Bir uzantı özellik için [`Rope`] için sağlar ek utility yöntemler.
pub trait RopeExt {
    /// Verilen 0 tabanlı satır indeksindeki satırın başlangıç ofseti.
    ///
    /// # Örnek
    ///
    /// ```
    /// use kavis_ui::{Rope, RopeExt};
    ///
    ...
    /// Verilen 0 tabanlı satır indeksindeki satırın bitiş ofseti (`\n` dahil).
    ///
    /// Satır sınır dışındaysa rope sonunu döndürür.
    ///
    /// ```
    /// use kavis_ui::{Rope, RopeExt};
    /// let rope = Rope::from("Hello\nWorld\r\nThis is a test 中文\nRope");
    ...
    /// Verilen 0 tabanlı satır indeksindeki satır dilimini döndürür; varsa `\r` dahil edilir, `\n` edilmez.
    ///
    /// ```
    /// use kavis_ui::{Rope, RopeExt};
    /// let rope = Rope::from("Hello\nWorld\r\nThis is a test 中文\nRope");
    ...
    /// bir slice satırlar içinde verilen aralık (0-temelli, bitiş hariç). döndürür.
    ///
    /// Aralık sınır dışındaysa geçerli aralığa sınırlandırılır.
    ///
    /// ```
    /// use kavis_ui::{Rope, RopeExt};
    /// let rope = Rope::from("Hello\nWorld\r\nThis is a test 中文\nRope");
    ...
    /// bir iterator üzerinde tüm satırlar içinde rope. döndürür.
    ///
    /// Each satır slice dahil eder `\r` ise present, ama değil `\n`.
    ///
    /// ```
    /// use kavis_ui::{Rope, RopeExt};
    /// let rope = Rope::from("Hello\nWorld\r\nThis is a test 中文\nRope");
    ...
    /// sayı satırlar içinde rope döndürür.
    ///
    /// ```
    /// use kavis_ui::{Rope, RopeExt};
    /// let rope = Rope::from("Hello\nWorld\r\nThis is a test 中文\nRope");
    ...
    /// uzunluk satır (0-temelli) içinde karakterler, dahil `\r` ise present, ama değil `\n` döndürür.
    ///
    /// Satır sınır dışındaysa 0 döndürür.
    ///
    /// ```
    /// use kavis_ui::{Rope, RopeExt};
    /// let rope = Rope::from("Hello\nWorld\r\nThis is a test 中文\nRope");
    ...
    /// Replace metin içinde verilen bayt aralık ile yeni metin.
    ///
    /// # Panikler
    ///
    /// - Aralık karakter sınırında değilse.
    /// - Aralık sınır dışındaysa.
    ///
    /// ```
    /// use kavis_ui::{Rope, RopeExt};
    ...
    /// Verilen bayt ofsetindeki karakteri döndürür.
    ///
    /// - Ofset çok baytlı bir karakterin ortasındaysa panic üretir.
    /// - Ofset sınır dışındaysa None döndürür.
    fn char_at(&self, offset: usize) -> Option<char>;

    /// Verilen 0 tabanlı satır/sütun [`konum`] değerinden bayt ofsetini döndürür.
    ///
    /// Sütun karakter cinsindendir.
    fn position_to_offset(&self, line_col: &Position) -> usize;

    /// Verilen bayt ofsetinden 0 tabanlı satır/sütun [`konum`] değerini döndürür.
    ///
    /// Sütun karakter cinsindendir.
    fn offset_to_position(&self, offset: usize) -> Position;

    /// Verilen bayt ofsetinden nokta (satır, sütun) döndürür.
    ///
    /// Sütun bayt cinsindendir.
    fn offset_to_point(&self, offset: usize) -> Point;

    /// Verilen noktadan (satır, sütun) bayt ofsetini döndürür.
    ///
    /// Sütun 0 tabanlıdır ve bayt cinsindendir.
    fn point_to_offset(&self, point: Point) -> usize;

    /// Verilen 0 tabanlı bayt ofsetindeki kelimenin bayt aralığını döndürür.
    fn word_range(&self, offset: usize) -> Option<Range<usize>>;

    /// Verilen 0 tabanlı bayt ofsetindeki kelimeyi döndürür.
    fn word_at(&self, offset: usize) -> String;

    /// ofset içinde UTF-16 için bayt ofset (0-temelli). dönüştürür.
    ///
    /// Runs içinde O(log N) time.
    fn offset_utf16_to_offset(&self, offset_utf16: usize) -> usize;

    /// bayt ofset (0-temelli) için ofset içinde UTF-16. dönüştürür.
    ///
    /// Runs içinde O(log N) time.
    fn offset_to_offset_utf16(&self, offset: usize) -> usize;

    /// Karakter sınırına göre kırpılmış ofset döndürür.
    ///
    /// - Bias::Left ise ve karakter sınırındaysa ix - 1 döndürür;
    /// - Bias::Right ise ve karakter sınırındaysa ix + 1 döndürür;
    /// - Otherwise döndürür ix.
    ///
    /// ```
    /// use kavis_ui::{Rope, RopeExt};
    /// use sum_tree::Bias;
    ...
    /// ofset içinde karakterler için bayt ofset (0-temelli). dönüştürür.
    ///
    /// Run içinde O(n) time.
    ///
    /// # Örnek
    ...
    /// bayt ofset (0-temelli) için ofset içinde karakterler. dönüştürür.
    ///
    /// Run içinde O(n) time.
    ///
    /// # Örnek
