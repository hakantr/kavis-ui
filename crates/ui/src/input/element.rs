...
/// bir bayt ofset içinde original metin için bir bayt ofset içinde maskelenmiş gösterim metin. dönüştürür.
///
/// Maskelenmiş metin, özgün metindeki her karakter için bir kez tekrarlanan `MASK_CHAR` değerinden oluşur.
/// Since `MASK_CHAR` may olmak multi-bayt içinde UTF-8, bayt ofset içinde maskelenmiş metin olur
...
/// Yerleşim bilgi için katlama simgeler.
...
impl TextElement {
    ...
    /// yer tutucu metin girdi alan ayarlar.
    ...
    /// - imleç sınırlar
    /// - kaydırma ofset
    /// - geçerli satır indeks (Hayır yalnızca görünür satırlar, ama tüm satırlar)
    ///
    /// Bu yöntem ayrıca imleç kaydırmasını izlemek için gerekli güncellemeleri yapar.
    fn layout_cursor(
        &self,
        ...
        if state.masked {
            ...
            && let Some(offset) = state.mask_pattern.first_editable_display_offset()
        {
            selected_range = (offset..offset).into();
            cursor = offset;
        }
    ...
    /// Yerleşim eşleşme aralık için bir yol.
    ...
    /// görünür aralık satırlar içinde görünüm alanı. hesaplar.
    ///
    /// Döndürür
    ///
    /// - visible_range: görünür aralık sarmalanmamış satırlara göre 0 tabanlıdır.
    /// - visible_buffer_lines: Indices non-gizlenir tampon satırlar içinde görünür aralık.
    /// - visible_top: üst konum ilk görünür satır içinde kaydırma görünüm alanı.
    ...
    /// (line_number_width, line_number_len) değerlerini döndürür.
    ...
    /// Yerleşim shaped satırlar için boşluk göstergeler (boşluk ve sekme).
    ///
    /// `WhitespaceIndicators` ile şekillendirilmiş satırlar için boşluk ve sekme karakterlerini döndürür.
    ...
    /// Satır içi tamamlama hayalet satırlarının çizimini hesaplar.
    ///
    /// (first_line, ghost_lines) yerde: döndürür.
    /// - first_line: Shaped metin için ilk satır (goes sonra imleç üzerinde aynı satır)
    /// - ghost_lines: Shaped satırlar için subsequent satırlar (shift içerik down)
    ...
    /// (line_number_width, line_number_len) değerlerini döndürür.
    /// Katlama simgesi hitboxlarını prepaint aşamasında yerleştirir.
    ///
    /// Bu, satır numaralarının sağına konumlandırılan katlama simge alanı için hitbox oluşturur.
    /// Panic oluşmasını önlemek için simgeler burada oluşturulur ve prepaint edilir.
    fn layout_fold_icons(
        ...
        for (ix, info) in fold_infos.iter().enumerate() {
            ...
            let mut icon = Dugme::new(("fold", ix))
                .ghost()
    ...
    /// Prepaint hitboxlarını kullanarak katlama simgelerini çizer.
    ///
    /// Bu handles:
    /// - Rendering katlama simgeler (chevron-sağ için katlanmış, chevron-down için genişletilmiş)
    /// - Mouse tıklama handling için açıp kapatma katlama durum
    /// - imleç stil değişimler üzerinde üzerine gelme
    /// - Yalnızca gösterir simge üzerinde üzerine gelme veya için geçerli satır
    fn paint_fold_icons(
        &mut self,
    ...
    fn layout_lines(
        ...
        for (vi, &buffer_line) in last_layout.visible_buffer_lines.iter().enumerate() {
            ...
            let line_item = buffer_lines
                .get(buffer_line)
                .expect("satır sarmalayıcıda mevcut olmalı");
...
pub(super) struct PrepaintState {
    /// satırlar entire satırlar.
    last_layout: LastLayout,
    /// satırlar yalnızca içerir görünür satırlar içinde görünüm alanı, temelli üzerinde `visible_range`.
    ///
    /// Alt değer yumuşak satırlardır.
    line_numbers: Option<Vec<SmallVec<[ShapedLine; 1]>>>,
    /// Boyut scrollable alan ile entire satırlar.
    scroll_size: Size<Pixels>,
    cursor_bounds: Option<Bounds<Pixels>>,
    cursor_scroll_offset: Point<Pixels>,
    /// Satır indeksi (sıfır tabanlı); sarma yoktur, imleçle aynı satırdır.
    current_row: Option<usize>,
    ...
    bounds: Bounds<Pixels>,
    /// Fold simge yerleşim veri
    fold_icon_layout: FoldIconLayout,
    // Inline completion rendering data
    /// Şekillendirilmiş hayalet satırları imleç satırından sonra çizer (2+ tamamlama satırı).
    ghost_lines: Vec<ShapedLine>,
    /// First satır satır içi tamamlama (çizilmiş sonra imleç üzerinde aynı satır)
    ghost_first_line: Option<ShapedLine>,
    ghost_lines_height: Pixels,
}

impl PrepaintState {
    /// imleç sınırlar adjusted için kaydırma ofset, ise kullanılabilir. döndürür.
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let mask_display = if state.masked {
            None
        } else {
            state.mask_pattern.display(&text.to_string())
        };
        let (display_text, text_color) = if let Some(mask_display) = &mask_display {
            (Rope::from(mask_display.text.as_str()), fg)
        } else if is_empty {
            (
        ...
        let mut runs = if let Some(mask_display) = &mask_display {
            mask_display
                .runs
                .iter()
                .map(|mask_run| TextRun {
                    len: mask_run.range.len(),
                    font: style.font(),
                    color: if mask_run.is_placeholder {
                        cx.theme().muted_foreground
                    } else {
                        fg
                    },
        ...
        if !state.error_ranges.is_empty() {
            runs = split_runs_by_fg_segments(&runs, &state.error_ranges, cx.theme().danger);
        }
        ...
        // The Kavis UI is a collection of UI components for
        // GPUI framework, including Dugme, Input, OnayKutusu, Radyo,
        // Dropdown, Sekme, and more...
        //
    ...
    fn paint(
        ...
        // Set KokGorunum focused_input when self is focused
        if focused {
            let state = self.state.clone();
            if KokGorunum::read(window, cx).focused_input.as_ref() != Some(&state) {
                KokGorunum::update(window, cx, |root, _, cx| {
        ...
        window.on_next_frame({
            let state = self.state.clone();
            move |window, cx| {
                if !focused && KokGorunum::read(window, cx).focused_input.as_ref() == Some(&state) {
                    KokGorunum::update(window, cx, |root, _, cx| {
...
/// Yer tutucu metni gösterim satırlarına böler ve her satırdaki çalışmaları kırpar.
...
/// runs için verilen aralık döndürür.
///
/// Aralık, sarılmış satırın bayt aralığıdır.
...
fn split_runs_by_fg_segments(
    runs: &[TextRun],
    fg_segments: &[Range<usize>],
    color: Hsla,
) -> Vec<TextRun> {
    let mut result = vec![];
    let mut cursor = 0;
    ...
    for run in runs {
        let run_start = cursor;
        let run_end = cursor + run.len;
        let mut local_cursor = run_start;

        for segment in fg_segments {
            if segment.end <= local_cursor || segment.start >= run_end {
                continue;
            }

            if segment.start > local_cursor {
                result.push(TextRun {
                    len: segment.start - local_cursor,
                    ..run.clone()
                });
            }

            let segment_end = segment.end.min(run_end);
            result.push(TextRun {
                len: segment_end - segment.start.max(local_cursor),
                color,
                ..run.clone()
            });
    ...
    result.into_iter().filter(|run| run.len > 0).collect()
}
