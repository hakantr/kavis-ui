...
use crate::highlighter::TaniKumesi;
...
pub struct Enter {
    /// İkincil onay olup olmadığını belirtir.
    pub secondary: bool,
}
...
/// Boşluk ve sekme karakterlerini çizmek için boşluk göstergeleri.
#[derive(Clone, Default)]
pub(crate) struct WhitespaceIndicators {
    /// Shaped satır için boşluk karakter gösterge (•)
    pub(crate) space: ShapedLine,
    /// Shaped satır için sekme karakter gösterge (→)
    pub(crate) tab: ShapedLine,
}

#[derive(Clone)]
pub(super) struct LastLayout {
    /// Görünüm alanındaki görünür aralık (sarmasız) satırları; değer 0 tabanlı satır indeksidir.
    /// Bu tampon satır aralık olan encompasses tüm görünür satırlar.
    pub(super) visible_range: Range<usize>,
    /// liste görünür tampon satır indeksler (excludes gizlenir/katlanmış satırlar).
    /// `lines` ile paraleldir: `visible_buffer_lines[i]`, `lines[i]` öğesinin tampon satır indeksidir.
    pub(super) visible_buffer_lines: Vec<usize>,
    /// Rope içindeki her görünür tampon satırının bayt ofseti (visible_buffer_lines/satırlar ile paralel).
    pub(super) visible_line_byte_offsets: Vec<usize>,
    /// ilk görünür satır üst konum içinde kaydırma görünüm alanı.
    pub(super) visible_top: Pixels,
    /// aralık bayt ofset görünür satırlar.
    pub(super) visible_range_offset: Range<usize>,
    /// son yerleşim satırlar (Yalnızca görünür satırları içerir; gizli satırlar için boş girdi yoktur).
    pub(super) lines: Rc<Vec<LineLayout>>,
    /// Metin yerleşimi için line_height; InputElement çizildiğinde değişir.
    pub(super) line_height: Pixels,
    /// Metin yerleşimi için sarma genişliği; InputElement çizildiğinde değişir.
    pub(super) wrap_width: Option<Pixels>,
    /// Metin yerleşimi için satır numarası alanı genişliği; satır numarası yoksa değer 0px olur.
    pub(super) line_number_width: Pixels,
    /// imleç konum (üst, sol) içinde pikseller.
    pub(super) cursor_bounds: Option<Bounds<Pixels>>,
    /// metin hizalar metin yerleşim.
    pub(super) text_align: TextAlign,
    /// içerik genişlik metin yerleşim.
    pub(super) content_width: Pixels,
}

impl LastLayout {
    /// satır yerleşim için verilen tampon satır (0-temelli) döndürür.
    ///
    /// Satırı bulmak için `visible_buffer_lines` üzerinde ikili arama kullanır.
    /// Satır görünür değilse (aralık dışında veya katlanmışsa) None döndürür.
    pub(crate) fn line(&self, row: usize) -> Option<&LineLayout> {
        let pos = self.visible_buffer_lines.binary_search(&row).ok()?;
        self.lines.get(pos)
    }

    /// hizalama ofset için verilen satır genişlik döndürür.
...
/// InputState için korur editing durum [`super::girdi`].
pub struct InputState {
    ...
    /// aralık içinde UTF-8 uzunluk için seçili metin.
    ///
    /// - "Hello 世界💝" = 16
    /// - "💝" = 4
    pub(super) selected_range: Selection,
    pub(super) search_panel: Option<Entity<AramaPaneli>>,
    pub(super) searchable: bool,
    /// aralık için save seçili kelime, kullanmak için korur kelime aralık olduğunda sürükleme taşıma.
    pub(super) selected_word_range: Option<Selection>,
    pub(super) selection_reversed: bool,
    /// İşaretli aralık, IME yazımı sırasında metne geçici olarak eklenir.
    pub(super) ime_marked_range: Option<Selection>,
    pub(super) last_layout: Option<LastLayout>,
    pub(super) last_cursor: Option<usize>,
    /// girdi kapsayıcı sınırlar
    ...
    /// Bu flag tells renderer için prefer bitiş geçerli görsel satır.
    ...
    pub(crate) error_ranges: Vec<Range<usize>>,
    ...
    ///
    /// [`Self::context_menu_builder`] içinde bağlam menüsü oluşturucu tanımlıysa bu değer yok sayılır.
    pub(super) enable_context_menu: bool,

    /// Şu anda bir tamamlama öğesi eklenip eklenmediğini belirten bayrak.
    pub(super) completion_inserting: bool,
    pub(super) hover_popover: Option<Entity<HoverPopover>>,
    /// LSP tanımlar locations için "Go için Definition" feature.
    pub(super) hover_definition: HoverDefinition,

    pub lsp: Lsp,

    /// Bir flag için indicate ise biz sahip bir pending günceller için metin.
    ///
    /// true ise çizimden önce bazı güncellemeleri çağırır (örneğin LSP veya sözdizimi vurgulama).
    _pending_update: bool,
    /// Bir flag için indicate ise biz olmalı ignore sonraki tamamlama olay.
    pub(super) silent_replace_text: bool,
    /// Bir flag için indicate ise biz olmalı emit InputEvents.
    pub(super) emit_events: bool,

    /// Yukarı/aşağı hareketlerde imlecin yatay sütununu (x koordinatı) hatırlamak için tutulur.
    ///
    /// İlk öğe x koordinatıdır (Pixels); tercih edilen kullanım budur.
    /// İkinci öğe sütundur (usize); bu değer yedek olarak kullanılır.
...
impl InputState {
    /// Bir girdi durum ile varsayılan [`InputMode::SingleLine`] mod. oluşturur.
    ///
    /// Bakınız ayrıca: [`Self::multi_line`], [`Self::auto_grow`] ayarlamak için diğer mod.
    ...
    /// girdi için kullanım multi satır mod ayarlar.
    ///
    /// Varsayılan satır sayısı 2dir.
    pub fn multi_line(mut self, multi_line: bool) -> Self {
        self.mode = self.mode.multi_line(multi_line);
        self
    }

    /// girdi için kullanım [`InputMode::AutoGrow`] mod ile minimum, maksimum satırlar sınır ayarlar.
    pub fn auto_grow(mut self, min_rows: usize, max_rows: usize) -> Self {
        self.mode = InputMode::auto_grow(min_rows, max_rows);
        self
    }

    /// girdi için kullanım [`InputMode::CodeEditor`] mod ayarlar.
    ///
    ...
    /// - indent_guides: true
    ///
    /// `highlighter` None ise varsayılan vurgulayıcı kullanılır.
    ///
    /// Kod düzenleyici basit kod düzenleme veya gösterim için yardımcıdır; tam özellikli bir kod düzenleyici değildir.
    ///
    /// ## Özellikler
    ///
    /// - Syntax Highlighting
    /// - Auto Indent
    /// - Satır Sayı
    /// - Large metin destek, up için 50K satırlar.
    ...
    /// Sağ tıklamada gösterilen bağlam menüsünün etkin olup olmadığını ayarlar.
    ///
    ...
    /// Bu girdinin aranabilir olup olmadığını ayarlar. Varsayılan false değeridir (kod düzenleyicide true).
    ...
    /// Yer tutucuyu ayarlar.
    pub fn placeholder(mut self, placeholder: impl Into<SharedString>) -> Self {
        self.placeholder = placeholder.into();
        self
    }

    /// etkinleştirir/devre dışı bırakır kod katlama, yalnızca için [`InputMode::CodeEditor`] mod ayarlar.
    ///
    /// Varsayılan: true
    ...
    /// Çalışma zamanında kod katlamayı ayarlar; yalnızca [`InputMode::CodeEditor`] modu içindir.
    ///
    /// Devre dışı bırakıldığında tüm mevcut katlamalar temizlenir.
    ...
    /// etkinleştirir/devre dışı bırakır satır sayı, yalnızca için [`InputMode::CodeEditor`] mod ayarlar.
    ...
    /// satır sayı, yalnızca için [`InputMode::CodeEditor`] mod ayarlar.
    ...
    /// sayı satırlar için multi-satır Textarea ayarlar.
    ///
    /// Bu yalnızca `multi_line` true olarak ayarlandığında kullanılır.
    ///
    /// varsayılan: 2
    ...
    /// vurgulayıcı dil için [`InputMode::CodeEditor`] mod ayarlar.
    ...
    #[inline]
    pub fn diagnostics(&self) -> Option<&TaniKumesi> {
        self.mode.diagnostics()
    }

    #[inline]
    pub fn diagnostics_mut(&mut self) -> Option<&mut TaniKumesi> {
    ...
    /// olan satır ve sub-satır verilen ofset belongs için, boyunca ile konum içinde olan sub-satır. bulur.
    ///
    /// Döndürür:
    ///
    /// - Ofseti içeren satır indeksi (sıfır tabanlı).
    /// - Ofseti içeren satır içindeki alt satır indeksi (sıfır tabanlı).
    /// - konum ofset.
    ...
    /// metin girdi alan ayarlar.
    ///
    /// selection_range de 0.0 değerine sıfırlanır.
    pub fn set_value(
        &mut self,
    ...
    /// Geçerli imleç konumuna metin ekler.
    ///
    /// İmleç eklenen metnin sonuna taşınır.
    pub fn insert(
        &mut self,
    ...
    /// Geçerli imleç konumundaki metni değiştirir.
    ///
    /// İmleç değiştirilen metnin sonuna taşınır.
    pub fn replace(
        &mut self,
        text: impl Into<SharedString>,
        window: &mut Window,
    ...
    /// ile devre dışı mod ayarlar.
    ///
    /// Bakınız ayrıca: [`Self::set_disabled`], [`Self::is_disabled`].
    #[allow(unused)]
    ...
    /// ile password maskelenmiş durum ayarlar.
    ///
    /// Yalnızca için [`InputMode::SingleLine`] mod.
    ...
    /// password maskelenmiş durum girdi alan ayarlar.
    ///
    /// Yalnızca için [`InputMode::SingleLine`] mod.
    ...
    /// true için temizler girdi ile pressing Escape key ayarlar.
    pub fn clean_on_escape(mut self) -> Self {
        self.clean_on_escape = true;
        self
    }

    /// Çok satırlı girdi için yumuşak sarma modunu ayarlar. Varsayılan true değeridir.
    ...
    /// olup olmadığını göstermek için boşluk karakterler ayarlar.
    pub fn show_whitespaces(mut self, show: bool) -> Self {
        self.show_whitespaces = show;
        self
    }

    /// Çok satırlı girdi için yumuşak sarma modunu günceller. Varsayılan true değeridir.
    ...
    /// olup olmadığını göstermek için boşluk karakterler. günceller.
    pub fn set_show_whitespaces(&mut self, show: bool, _: &mut Window, cx: &mut Context<Self>) {
        self.show_whitespaces = show;
        cx.notify();
    }

    /// regular expression desen girdi alan ayarlar.
    ///
    /// Yalnızca için [`InputMode::SingleLine`] mod.
    ...
    /// regular expression desen girdi alan ile referans ayarlar.
    ///
    /// Yalnızca için [`InputMode::SingleLine`] mod.
    pub fn set_pattern(
        &mut self,
    ...
    /// validation fonksiyon girdi alan ayarlar.
    ///
    /// Yalnızca için [`InputMode::SingleLine`] mod.
    ...
    /// true ise girdinin sağında spinner gösterir.
    ///
    /// Yalnızca için [`InputMode::SingleLine`] mod.
    ...
    /// varsayılan değer girdi alan ayarlar.
    ...
    /// değer girdi alan döndürür.
    pub fn value(&self) -> SharedString {
        SharedString::new(self.text.to_string())
    }

    /// portion değer içinde girdi alan olan döndürür.
    /// seçili ile kullanıcı
    pub fn selected_value(&self) -> SharedString {
        SharedString::new(self.selected_text().to_string())
    }

    /// değer olmadan maske döndürür.
    pub fn unmask_value(&self) -> SharedString {
        self.mask_pattern.unmask(&self.text.to_string()).into()
    }

    /// metin [`Rope`] girdi alan döndürür.
    pub fn text(&self) -> &Rope {
        &self.text
    }

    /// (0-temelli) [`konum`] imleç döndürür.
    pub fn cursor_position(&self) -> Position {
        let offset = self.cursor();
        self.text.offset_to_position(offset)
    }

    /// (0-temelli) [`konum`] imleç ayarlar.
    ///
    /// İmleci belirtilen satır ve sütuna taşır ve seçim aralığını günceller.
    ...
    /// başlangıç ofset önceki kelime döndürür.
    ...
    /// sonraki bitiş ofset sonraki kelime döndürür.
    ...
    /// başlangıç satır bayt ofset imleç. döndürür.
    ///
    /// Yumuşak sarma etkin olduğunda ilk basış görsel satırın başına gider,
    /// İkinci basış (zaten görsel başlangıçtayken) mantıksal satır başlangıcına gider.
    ...
    /// bitiş satır bayt ofset imleç. döndürür.
    ///
    /// Yumuşak sarma etkin olduğunda ilk basış görsel satırın sonuna gider,
    /// İkinci basış (zaten görsel bitişteyken) mantıksal satır bitişine gider.
    ...
    /// başlangıç satır seçim başlangıç veya bitiş (minimum değer). döndürür.
    ///
    /// Bu, her zaman seçimin ilk satırını alacağı anlamına gelir.
    ...
    /// indent metin sonraki satır. döndürür.
    ///
    /// To alır geçerli ve sonraki satır indent, için döndürür daha fazla depth bir.
    ...
    pub(super) fn backspace(&mut self, _: &Backspace, window: &mut Window, cx: &mut Context<Self>) {
        if self.selected_range.is_empty() {
            ...
            if let Some(range) = self
                .mask_pattern
                .previous_editable_range(&self.text.to_string(), cursor)
            {
    ...
    pub(super) fn delete(&mut self, _: &Delete, window: &mut Window, cx: &mut Context<Self>) {
        if self.selected_range.is_empty() {
            ...
            if let Some(range) = self
                .mask_pattern
                .next_editable_range(&self.text.to_string(), cursor)
            {
                self.selected_range = range.into();
            } else {
    ...
    /// Verilen ofseti görünür yapmak için kaydırır.
    ///
    /// Yön bilgisi doluysa kenarları aynı tarafta tutar.
    ...
    /// bayt ofset imleç. döndürür.
    ///
    /// Ofset UTF-8 ofsetidir.
    ...
    /// Son yerleşimi yapılmış görünüm alanındaki görünür satır aralığı; ilk yerleşimden önce `None` olur.
    ...
    /// geçerli seçim olarak bir bayt aralık içine metin döndürür.
    ///
    /// Aralık boşsa (`start == end`) seçili metin yoktur;
    /// bu durumda ofset `imleç()` değerine eşittir. Bayt ofsetleri
    /// içinde underlying rope's bayt units.
    ...
    /// bir y offsetted nokta için satır origin. döndürür.
    /// Geçerli imleç konumundan verilen ofsete kadar metni seçer.
    ///
    /// Ofset UTF-8 ofsetidir.
    ///
    /// Doğru ofseti almak için self.next_boundary veya self.previous_boundary kullanılmasını sağlar.
    ...
    /// Unselects şu anda seçili metin.
    ...
    /// Ofset gizli (katlanmış) satıra düşerse geriye doğru önceki görünür satırın sonuna sınırlar.
    /// katlama başlık satır (son görünür konum önce katlama).
    ...
    /// Eğer ofset falls üzerinde bir gizlenir (katlanmış) satır, sınırlandırır forward için başlangıç nin
    /// katlama bitiş satır (ilk görünür konum sonra katlama).
    ...
    /// Girdi odaklı ve geçerli BlinkCursor görünür olduğunda InputElementin imleci çizmesi için true döndürür.
    ...
    fn on_blur(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        KokGorunum::update(window, cx, |root, _, _| {
    ...
    /// maske desen için formatting girdi metin ayarlar.
    ///
    /// Desen şunları içerebilir:
    /// - 9: Herhangi bir digit veya nokta
    /// - A: Herhangi bir letter
    /// - *: Herhangi bir karakter
    /// - Diğer karakterler değişmez maske karakterleri olarak ele alınır.
    ///
    /// Example: "(999)999-999" için phone sayılar
    ...
    pub fn set_error_ranges(&mut self, ranges: Vec<Range<usize>>, cx: &mut Context<Self>) {
        if self.error_ranges == ranges {
            return;
        }
    ...
    /// çizilmiş sınırlar için bir UTF-8 bayt aralık içinde geçerli girdi contents döndürür.
    ///
    /// İstenen aralık şu anda yerleşimde değilse veya görünür değilse `None` döndürür.
    ...
    /// Replace metin içinde aralık içinde silent.
    ///
    /// Bu olmayacak tetikleyici herhangi bir UI interaction, böyle olarak auto-tamamlama.
    ...
    /// tree-sitter sözdizimi ağacından katlama adaylarını günceller (tam çıkarım).
    /// Yalnızca ilk yüklemede veya dil değişimlerinde kullanılır.
    ...
    /// Yalnızca traverses düzenlenmiş bölge syntax ağaç bunun yerine tam ağaç.
    ...
    /// Spawn bir arka plan ayrıştırır sonra synchronous ayrıştırır timed out.
    ///
    /// Dropping döndürülen `Task` (stored içinde `parse_task`) cancels
    /// ayrıştırır, olan naturally debounces rapid edits.
    #[cfg(not(target_family = "wasm"))]
    fn dispatch_background_parse(
        ...
        let task = cx.spawn_in(window, async move |entity, cx| {
            let result = cx
                .background_executor()
                .spawn(async move {
                    let Some(config) = DilKaydi::singleton().language(&language) else {
                        return None;
                    };
                    ...
                    let injection_layers = if let Some(data) = injection_data {
                        crate::highlighter::SozdizimiVurgulayici::compute_injection_layers(
                            data, &new_tree, &text,
                        )
                    } else {
                        Default::default()
...
impl EntityInputHandler for InputState {
    ...
    /// Replace metin içinde aralık.
    ///
    /// - Yeni metin geçersizse değiştirme yapılmaz.
    /// - `range_utf16` sağlanmazsa geçerli seçili aralık kullanılır.
    fn replace_text_in_range(
        ...
        if self.mode.is_single_line() {
            ...
            if !self.mask_pattern.is_none() {
                let mut mask_text = self.mask_pattern.mask(&pending_text).to_string();
                let new_text_len =
                    (new_text.len() + mask_text.len()).saturating_sub(pending_text.len());
                new_offset = (range.start + new_text_len).min(mask_text.len());

                if new_text.is_empty() {
                    if let Some((trimmed_text, _)) =
                        self.mask_pattern.trim_trailing_separators(&mask_text)
                    {
                        mask_text = trimmed_text;
                        new_offset = new_offset.min(mask_text.len());
                    }
                } else if let Some((expanded_text, expanded_offset)) = self
                    .mask_pattern
                    .append_next_separator_after_cursor(&mask_text, new_offset)
                {
                    mask_text = expanded_text;
                    new_offset = expanded_offset;
                }
    ...
    /// İşaretli metin, IME yazımı sırasında geçici olarak eklenir.
    ...
    /// İçin kullanılır konum IME adaylar.
    fn bounds_for_range(
        &mut self,
        range_utf16: Range<usize>,
