...
pub(crate) enum InputMode {
    /// Bir plain metin girdi mod.
    PlainText {
        multi_line: bool,
        tab: TabSize,
        rows: usize,
    },
    /// Bir auto grow girdi mod.
    AutoGrow {
        rows: usize,
        min_rows: usize,
        max_rows: usize,
    },
    /// Bir kod düzenleyici girdi mod.
    CodeEditor {
        multi_line: bool,
        tab: TabSize,
        rows: usize,
        ...
        indent_guides: bool,
        folding: bool,
        highlighter: Rc<RefCell<Option<SozdizimiVurgulayici>>>,
        diagnostics: TaniKumesi,
...
#[allow(unused)]
impl InputMode {
    /// Bir plain girdi mod ile varsayılan ayarlar. oluşturur.
    ...
    /// Bir kod düzenleyici girdi mod ile varsayılan ayarlar. oluşturur.
    pub(super) fn code_editor(language: impl Into<SharedString>) -> Self {
        InputMode::CodeEditor {
            ...
            indent_guides: true,
            folding: true,
            diagnostics: TaniKumesi::new(&Rope::new()),
            parse_task: Rc::new(RefCell::new(None)),
        }
    }

    /// Oluşturur bir auto grow girdi mod ile verilen minimum ve maksimum satırlar.
    ...
    /// Mod kod düzenleyici, `folding: true` ve `multi_line: true` ise true döndürür.
    #[inline]
    ...
    /// At least 1 satır olmak döndürür.
    ...
    /// Mod [`InputMode::CodeEditor`] değilse false döndürür.
    #[inline]
    ...
    /// syntax vurgulayıcı ile yeni metin. günceller.
    ///
    /// `Some(PendingBackgroundParse)` olduğunda synchronous ayrıştırır döndürür.
    /// timed out ve çağıran olmalı dispatch bir arka plan ayrıştırır.
    /// Ayrıştırma tamamlandığında veya etkin vurgulayıcı yoksa `None` döndürür.
        ...
        match &self {
            InputMode::CodeEditor {
                ...
                if highlighter_ref.is_none() {
                    let new_highlighter = SozdizimiVurgulayici::new(language);
    ...
    #[allow(unused)]
    pub(super) fn diagnostics(&self) -> Option<&TaniKumesi> {
        match self {
    ...
    pub(super) fn diagnostics_mut(&mut self) -> Option<&mut TaniKumesi> {
        match self {
            InputMode::CodeEditor { diagnostics, .. } => Some(diagnostics),
            _ => None,
        }
    ...
    /// bir referans için vurgulayıcı (ise kullanılabilir) döndürür.
    pub(super) fn highlighter(&self) -> Option<&Rc<RefCell<Option<SozdizimiVurgulayici>>>> {
        match self {
