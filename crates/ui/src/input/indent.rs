...
pub struct TabSize {
    /// Varsayılan 2dir.
    pub tab_size: usize,
    /// true için kullanım `\t` olarak sekme indent ayarlar. Varsayılan: false olur
    pub hard_tabs: bool,
}
...
impl TabSize {
    ...
    /// sayı indent boyut satır içinde spaces.
...
impl TextElement {
    /// Olcum indent genişlik içinde pikseller için verilen sütun sayı.
...
impl InputState {
    /// Kod düzenleyici modunda girinti kılavuzlarının gösterilip gösterilmeyeceğini ayarlar. Varsayılan true değeridir.
    ///
    /// Yalnızca için [`InputMode::CodeEditor`] mod.
    ...
    /// indent guides içinde kod düzenleyici mod ayarlar.
    ///
    /// Yalnızca için [`InputMode::CodeEditor`] mod.
    ...
    /// sekme boyut için girdi ayarlar.
    ///
    /// Yalnızca için [`InputMode::PlainText`] ve [`InputMode::CodeEditor`] mod ile multi_line.
