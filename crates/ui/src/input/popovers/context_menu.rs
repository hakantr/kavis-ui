...
impl InputState {
    pub(crate) fn handle_right_click_menu(
        ...
        // Check if we are already in a deferred context (e.g., inside a AcilirKatman)
        // If so, don't show the context menu to prevent double-deferred panic
        if KureselDurum::global(cx).is_in_deferred_context() {
            return;
        }
            ...
            this.menu.update(cx, |menu, cx| {
                let new_menu = if let Some(builder) = &self.context_menu_builder {
                    ...
                    AcilirMenu::new(cx)
                        ...
                        .separator()
                        .menu(t!("Input.Secim All"), Box::new(input::SelectAll))
                };
