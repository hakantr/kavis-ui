    ...
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        h_flex()
            ...
            .when(item.detail.is_some(), |this| {
                this.child(
                    Etiket::new(item.detail.as_deref().unwrap_or("").to_string())
...
/// Bir bağlam menü için kod tamamlamalar ve kod eylemler.
...
impl CompletionMenu {
    /// Yeni bir `CompletionMenu` ile verilen ofset ve tamamlama öğeler oluşturur.
    ///
    /// NOT: Stack overflow oluşmaması için bu öğe InputState::yeni içinden çağrılmamalıdır.
    pub(crate) fn new(
        editor: Entity<InputState>,
        window: &mut Window,
        cx: &mut App,
        ...
        cx.new(|cx| {
            ...
            let list = cx.new(|cx| ListeDurumu::new(menu, window, cx));

            let _subscriptions =
                vec![
                    cx.subscribe(&list, |this: &mut Self, _, ev: &ListeOlayi, cx| {
                        match ev {
                            ListeOlayi::Confirm(_) => {
                                this.hide(cx);
                            }
                            _ => {}
                        }
    ...
    /// Gizler tamamlama menü ve sıfırlar tetikleyici başlangıç ofset.
    ...
    /// Henüz ayarlanmamışsa tetikleyici başlangıç ofsetini ayarlar.
            ...
            div()
                ...
                .child(
                    editor_popover("completion-menu", cx)
                        .max_w(max_width)
                        .min_w(px(120.))
                        .child(Liste::new(&self.list).max_h(MAX_MENU_HEIGHT)),
                )
