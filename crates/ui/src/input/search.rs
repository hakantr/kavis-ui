...
use crate::{
    Disableable, ElementExt, EtkinTema, Selectable, SimgeAdi, Sizable,
    actions::SelectUp,
...
const CONTEXT: &'static str = "AramaPaneli";
...
#[derive(Debug, Clone)]
pub struct AramaEsleyici {
    text: Rope,
    pub query: Option<AhoCorasick>,

    pub(super) matched_ranges: Rc<Vec<Range<usize>>>,
    pub(super) current_match_ix: usize,
    /// Is içinde replacing mod, ise true, sonraki günceller olmayacak sıfırlar geçerli eşleşme indeks.
    replacing: bool,
}
...
impl AramaEsleyici {
    ...
    /// kaynak metin ve re-eşleşme günceller.
    ...
    fn update_matches(&mut self) {
        ...
        if let Some(query) = &self.query {
            ...
            for query_match in matches.into_iter() {
                let query_match = query_match.expect("tümünü seç eylemi için sorgu eşleşmesi");
    ...
    /// Arama sorgusunu günceller ve geçerli eşleşme indeksini sıfırlar.
        ...
        if query.len() > 0 {
            self.query = Some(
                AhoCorasick::builder()
                    ...
                    .expect("AramaEsleyici içinde AhoCorasick sorgusu oluşturulamadı"),
            );
        } else {
            self.query = None;
        }
    ...
    /// geçerli eşleşme indeks temelli üzerinde verilen ofset. günceller.
...
impl Iterator for AramaEsleyici {
...
impl DoubleEndedIterator for AramaEsleyici {
...
pub(super) struct AramaPaneli {
    editor: Entity<InputState>,
    search_input: Entity<InputState>,
    replace_input: Entity<InputState>,
    case_insensitive: bool,
    replace_mode: bool,
    matcher: AramaEsleyici,
    input_width: Pixels,
...
impl InputState {
    /// Metin değiştiğinde arama eşleyicisini günceller.
    ...
    pub(super) fn on_action_search(
        ...
        let search_panel = match self.search_panel.as_ref() {
            ...
            None => AramaPaneli::new(cx.entity(), window, cx),
        };
...
impl AramaPaneli {
    ...
    pub(super) fn matcher(&self) -> Option<&AramaEsleyici> {
        if !self.open {
            return None;
        }
...
impl Focusable for AramaPaneli {
    fn focus_handle(&self, cx: &App) -> FocusHandle {
        self.search_input.read(cx).focus_handle.clone()
    }
}

impl Render for AramaPaneli {
        ...
        v_flex()
            ...
            .child(
                h_flex()
                        ...
                        div()
                            ...
                            .child(
                                Input::new(&self.search_input)
                                    ...
                                    .suffix(
                                        Dugme::new("case-insensitive")
                                            .selected(!self.case_insensitive)
                                            .xsmall()
                                            .compact()
                                            .ghost()
                                            .icon(SimgeAdi::CaseSensitive)
                    ...
                    .child(
                        Dugme::new("replace-mode")
                            .xsmall()
                            .ghost()
                            .icon(SimgeAdi::Replace)
...
mod tests {
    ...
    #[test]
    fn test_search() {
        let mut matcher = AramaEsleyici::new();
