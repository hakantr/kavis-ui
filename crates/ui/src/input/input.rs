...
/// `(arka plan, foreground)` renkler için girdi-gibi bileşenler. döndürür.
...
/// Bir metin girdi öğe bind için bir [`InputState`].
#[derive(IntoElement)]
pub struct Input {
...
impl Input {
    /// Yeni bir [`girdi`] öğe bind için [`InputState`] oluşturur.
    ...
    /// tam yükseklik girdi (Multi-satır yalnızca) ayarlar.
    pub fn h_full(mut self) -> Self {
        self.height = Some(relative(1.));
        self
    }

    /// yükseklik girdi (Multi-satır yalnızca) ayarlar.
    pub fn h(mut self, height: impl Into<DefiniteLength>) -> Self {
        self.height = Some(height.into());
        self
    }

    /// Girdi alanının görünümünü ayarlar; false ise kenarlık ve arka plan olmaz.
    pub fn appearance(mut self, appearance: bool) -> Self {
        self.appearance = appearance;
        self
    }

    /// bordered için girdi ayarlar. Varsayılan: true
    pub fn bordered(mut self, bordered: bool) -> Self {
        self.bordered = bordered;
        self
    }

    /// Girdinin odak kenarlığını ayarlar. Varsayılan true değeridir.
    ...
    /// için etkinleştirir açıp kapatma düğme için password maske durum ayarlar.
    pub fn mask_toggle(mut self) -> Self {
        self.mask_toggle = true;
        self
    }

    /// için devre dışı bırakır girdi alan ayarlar.
    pub fn disabled(mut self, disabled: bool) -> Self {
        self.disabled = disabled;
        self
    }

    /// Girdinin sekme indeksini ayarlar. Varsayılan 0dır.
    ...
    /// Bu yöntem olmalıdır sonra refine_style.
    fn render_editor(
            ...
            .child(div().flex_1().child(input_state.clone()).map(|this| {
                if let Some(last_layout) = state.last_layout.as_ref() {
                    ...
                    let scrollbar = if !state.soft_wrap {
                        KaydirmaCubugu::new(&state.scroll_handle)
                    } else {
                        KaydirmaCubugu::vertical(&state.scroll_handle)
                    };
                ...
                this.pr(self.size.input_px()).child(
                    h_flex()
                        ...
                        .when(state.loading, |this| {
                            this.child(DonerGosterge::new().color(cx.theme().muted_foreground))
                        })
