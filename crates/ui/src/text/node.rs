...
use super::{MetinGorunumuStili, utils::list_item_prefix};

/// block-seviye düğümler.
#[derive(Debug, Clone, PartialEq)]
pub(crate) enum BlockNode {
    /// Something gibi bir Div kapsayıcı içinde HTML.
    KokGorunum {
    ...
    Liste {
        /// Yalnızca ListeOgesi içerir; diğerleri yok sayılır.
        children: Vec<BlockNode>,
        ordered: bool,
        span: Option<Span>,
    },
    ListeOgesi {
        children: Vec<BlockNode>,
        spread: bool,
        /// Liste öğesinin işaretli olup olmadığını belirtir; None ise onay kutusu değildir.
        checked: Option<bool>,
        span: Option<Span>,
    },
    CodeBlock(CodeBlock),
    ...
    /// Kullanım için to_markdown alır ham tanım
    Definition {
...
impl BlockNode {
    ...
    /// Combine tüm alt öğeler, omitting empt üst öğe düğümler.
    pub(super) fn compact(self) -> BlockNode {
        match self {
            Self::KokGorunum { mut children, .. } if children.len() == 1 => {
                children.remove(0).compact()
            }
            _ => self,
        }
    }

    /// Span düğümünü döndürür.
    pub(super) fn span(&self) -> Option<Span> {
        match self {
            BlockNode::KokGorunum { span, .. } => *span,
            BlockNode::Paragraph(paragraph) => paragraph.span,
            BlockNode::Heading { span, .. } => *span,
            BlockNode::Blockquote { span, .. } => *span,
            BlockNode::Liste { span, .. } => *span,
            BlockNode::ListeOgesi { span, .. } => *span,
            BlockNode::CodeBlock(code_block) => code_block.span,
            BlockNode::Tablo(table) => table.span,
            BlockNode::Break { span, .. } => *span,
            BlockNode::Ayirici { span, .. } => *span,
    ...
    pub(super) fn selected_text(&self) -> String {
        ...
        match self {
            ...
            BlockNode::Liste { children, .. } => {
                for c in children.iter() {
                    text.push_str(&c.selected_text());
                }
            }
            BlockNode::ListeOgesi { children, .. } => {
                for c in children.iter() {
                    text.push_str(&c.selected_text());
                }
            }
...
pub struct LinkMark {
    ...
    /// isteğe bağlı tanımlayıcı için footnotes.
...
pub(crate) struct InlineNode {
    ...
    /// Metin stilleri; her demet metin aralığı ve stil içerir.
...
/// paragraph öğe, içerir çoklu metin düğümler.
///
/// Diğer öğelerden farklı olarak bu klonlanabilir; çünkü Node AST içinde kullanılır.
/// Seçim durumunu bu AST düğümlerinde saklarız.
#[derive(Debug, Clone, Default)]
pub(crate) struct Paragraph {
    pub(super) span: Option<Span>,
    pub(super) children: Vec<InlineNode>,
    /// link referanslar içinde bu paragraph, için kullanılır referans links.
    ///
    /// Anahtar tanımlayıcı, değer URLdir.
...
pub(crate) struct Tablo {
    pub(crate) children: Vec<TabloSatiri>,
...
#[derive(Debug, Clone, Default, PartialEq)]
pub(crate) struct TabloSatiri {
    pub children: Vec<TabloHucresi>,
}

#[derive(Debug, Clone, Default, PartialEq)]
pub(crate) struct TabloHucresi {
...
impl Paragraph {
    ...
    /// uzunluk alt öğeler metin. döndürür.
...
impl CodeBlock {
    /// dil kod block döndürür.
    pub fn lang(&self) -> Option<SharedString> {
        self.lang.clone()
    }

    /// kod içerik kod block döndürür.
    ...
    pub(crate) fn new(
        code: SharedString,
        lang: Option<SharedString>,
        highlight_theme: &VurguTemasi,
        span: Option<impl Into<Span>>,
    ) -> Self {
        let mut styles = vec![];
        if let Some(lang) = &lang {
            let mut highlighter = SozdizimiVurgulayici::new(&lang);
...
/// Bağlantı referanslarını içeren düğüm çizim bağlamı.
#[derive(Default, Clone)]
pub(crate) struct NodeContext {
    /// bayt ofset düğüm içinde original markdown metin.
    /// İçin kullanılır artımlı updates.
    pub(crate) offset: usize,
    pub(crate) link_refs: HashMap<SharedString, LinkMark>,
    pub(crate) style: MetinGorunumuStili,
                        ...
                        .when_some(image.link.clone(), |this, link| {
                            ...
                            this.cursor_pointer()
                                .tooltip(move |window, cx| {
                                    AracIpucu::new(title.clone()).build(window, cx)
                                })
...
impl BlockNode {
    /// düğüm için markdown biçim. dönüştürür.
    ///
    /// Bu için kullanılır generate markdown için test.
    #[allow(dead_code)]
    pub(crate) fn to_markdown(&self) -> String {
        match self {
            BlockNode::KokGorunum { children, .. } => children
                .iter()
...
impl BlockNode {
                        ...
                        match child {
                            BlockNode::Paragraph { .. } => {
                                let last_not_list = child_ix > 0
                                    && !matches!(children[child_ix - 1], BlockNode::Liste { .. });
    ...
    pub(crate) fn render_block(
        ...
        match self {
            BlockNode::KokGorunum { children, .. } => div()
                .id(("div", ix))
