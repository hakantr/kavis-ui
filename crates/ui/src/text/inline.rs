...
use crate::{EtkinTema, global_state::KureselDurum, input::Selection, text::node::LinkMark};

/// Bir satır içi öğe için kullanılır çizer bir satır içi metin ve destek seçilebilir.
///
/// MetinGorunumu içindeki tüm metinler (CodeBlock dahil) metin çizimi için bunu kullanır.
...
/// satır içi metin durum, kullanılır RefCell için korur seçim durum.
#[derive(Debug, Default, PartialEq)]
pub(crate) struct InlineState {
    hovered_index: Option<usize>,
    /// Seçimle eşleşen, gerçekten çizilen metin.
    pub(super) text: SharedString,
    pub(super) selection: Option<Selection>,
}

impl InlineState {
    /// Seçili metinde kullanmak üzere gerçekten çizilen metni saklar.
...
impl Inline {
    ...
    /// Verilen fare konumundaki bağlantıyı döndürür.
    ...
    fn layout_selections(
        ...
        let Some(text_view_state) = KureselDurum::global(cx).text_view_state() else {
...
/// Çok satırlı seçimleri dikkate alarak `pos` değerinin `sınırlar` içinde olup olmadığını kontrol eder.
