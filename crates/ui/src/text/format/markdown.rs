...
use crate::{
    ...
    text::{
        ...
        node::{
            ...
            Span, Tablo, TabloSatiri, TextMark,
        },
    },
};

/// Markdown metnini düğüm ağacına ayrıştırır.
///
/// TODO: `highlight_theme` seçeneğini kaldır; bu işlem çizim aşamasında yapılmalı.
pub(crate) fn parse(
    source: &str,
    cx: &mut NodeContext,
...
fn parse_table_row(table: &mut Tablo, node: &mdast::TableRow, cx: &mut NodeContext) {
    let mut row = TabloSatiri::default();
...
fn parse_table_cell(row: &mut node::TabloSatiri, node: &mdast::TableCell, cx: &mut NodeContext) {
    let mut paragraph = Paragraph::default();
    node.children.iter().for_each(|c| {
        parse_paragraph(&mut paragraph, c, cx);
    });
    let table_cell = node::TabloHucresi {
        children: paragraph,
        ..Default::default()
    };
...
fn parse_paragraph(paragraph: &mut Paragraph, node: &mdast::Node, cx: &mut NodeContext) -> String {
    ...
    match node {
            ...
            Ok(el) => {
                    ...
                    .unwrap_or(false)
                {
                    ...
                    if cfg!(debug_assertions) {
                        tracing::warn!("desteklenmeyen satır içi html etiketi: {:#?}", el);
                    }
                }
            }
            Err(err) => {
                if cfg!(debug_assertions) {
                    tracing::warn!("html ayrıştırılamadı: {:#?}", err);
                }
        ...
        _ => {
            if cfg!(debug_assertions) {
                tracing::warn!("desteklenmeyen satır içi düğüm: {:#?}", node);
            }
        }
    }
...
fn ast_to_node(
    ...
    match value {
        Node::Root(_) => unreachable!("node::KokGorunum should be handled separately"),
        Node::Paragraph(val) => {
            let mut paragraph = Paragraph::default();
            val.children.iter().for_each(|c| {
                parse_paragraph(&mut paragraph, c, cx);
        ...
        Node::Html(val) => match super::html::parse(&val.value, cx) {
            Ok(el) => BlockNode::KokGorunum {
                children: el.blocks,
                span: new_span(val.position, cx),
            },
            Err(err) => {
                if cfg!(debug_assertions) {
                    tracing::warn!("html ayrıştırılırken hata: {:#?}", err);
                }
        ...
        Node::ThematicBreak(val) => BlockNode::Ayirici {
            span: new_span(val.position, cx),
        },
        Node::Table(val) => {
            let mut table = Tablo::default();
        ...
        _ => {
            if cfg!(debug_assertions) {
                tracing::warn!("desteklenmeyen düğüm: {:#?}", value);
            }
            BlockNode::Unknown
        }
    }
