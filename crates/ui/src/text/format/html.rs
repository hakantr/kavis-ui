...
use crate::text::node::{
    self, BlockNode, ImageNode, InlineNode, LinkMark, NodeContext, Paragraph, Tablo, TabloSatiri,
    TextMark,
};
...
/// HTML içine AST Node. ayrıştırır.
...
/// Stil özelliklerini HashMap olarak döndürür.
/// TODO: Stil niteliğini ayrıştırmak için cssparser kullan.
...
/// Stil niteliğinden uzunluk değerini ayrıştırır.
///
/// Yüzde değerler göreli uzunluğa dönüştürülür.
/// Diğer değerler piksele dönüştürülür.
...
/// Genişlik ve yüksekliği niteliklerden veya stil niteliğinden ayrıştırarak döndürür.
...
fn parse_table_row(table: &mut Tablo, node: &Rc<Node>) {
...
/// Metni kırpar, ancak en az bir boşluk bırakır.
///
/// - Before: " \r\n Hello world \t "
/// - After: " Hello world "
#[allow(dead_code)]
            ...
            local_name!("img") => {
                let Some(src) = attr_value(attrs, local_name!("src")) else {
                    if cfg!(debug_assertions) {
                        tracing::warn!("Görsel düğümünde src özniteliği eksik");
                    }
                    return;
                };
    ...
    match node.data {
        ...
        NodeData::Element {
            ...
            local_name!("table") => {
                ...
                let table = BlockNode::Tablo(table);
                if children.len() > 0 {
                    children.push(table);
                    Some(BlockNode::KokGorunum {
                        children,
