...
/// Minify davranışını tanımlar.
#[allow(dead_code)]
pub(crate) trait Minify {
    /// Kaynağı küçültür ve küçültülmüş HTML5 döndürür.
    ///
    /// # Errors
    ///
    /// Girdi okuyucusundan okuma veya
    /// çıktı yazıcısına yazma başarısız olursa `Err` döndürür.
    fn minify(&self) -> Result<Vec<u8>, io::Error>;
}

/// HTML girdisini hedef yazıcıya küçültülmüş olarak yazar.
/// HTML5 çıktısı üretir; HTML5 olmayan girdi HTML5 biçimine dönüştürülür.
///
/// # Errors
///
/// Girdi okuyucusundan okuma veya
...
/// `io::Write` için minifier uygulaması.
...
/// Düğümün konumsal bağlamını tutar.
struct Context<'a> {
    parent: &'a Node,
...
impl<'a> Context<'a> {
    /// Boşluğun kırpılıp kırpılmayacağını belirler.
    /// Basit HTML5 boşluk daraltma kurallarını kullanır.
...
where
    W: io::Write,
{
    /// Yeni bir `Minifier` örnek oluşturur.
    #[inline]
    ...
    /// Öğeler arasında ve boşluğun korunmadığı metin içinde boşlukları daraltır.
    ...
    /// Verilen okuyucu girdisini küçültür.
    ///
    /// # Errors
    ///
    /// Çıktı yazıcısına yazılamazsa `Err` döndürür.
    #[inline]
    #[allow(dead_code)]
    ...
    /// Başlangıç ve bitiş etiketlerinin atlanıp atlanamayacağını belirler.
    /// `collapse_whitespace` etkinse boşluk kuralları yok sayılır.
    ...
    /// Daraltılmış boşluk içermeyen metin gibi blok içeriklerini verimli şekilde yazar;
    /// bu durumda tek yazma işlemi yeterlidir.
