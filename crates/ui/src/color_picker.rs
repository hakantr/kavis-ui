...
const CONTEXT: &'static str = "RenkSecici";
...
/// Olaylar yayılan ile [`RenkSecici`].
#[derive(Clone)]
pub enum RenkSeciciOlayi {
    Change(Option<Hsla>),
}
...
#[derive(Clone)]
struct HslaSliders {
    hue: Entity<KaydiriciDurumu>,
    saturation: Entity<KaydiriciDurumu>,
    lightness: Entity<KaydiriciDurumu>,
    alpha: Entity<KaydiriciDurumu>,
}
...
impl RenkSeciciDurumu {
    /// Yeni bir [`RenkSeciciDurumu`] oluşturur.
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let mut _subscriptions = vec![
            ...
            cx.subscribe_in(
                &hsla_sliders.hue,
                window,
                |this, _, _: &KaydiriciOlayi, window, cx| {
    ...
    fn update_value(
        ...
        if emit {
            cx.emit(RenkSeciciOlayi::Change(value));
        }
        cx.notify();
    }
    ...
    fn update_value_from_slider(
        ...
        if emit {
            cx.emit(RenkSeciciOlayi::Change(Some(value)));
        }
        cx.notify();
    }
...
impl EventEmitter<RenkSeciciOlayi> for RenkSeciciDurumu {}

impl Render for RenkSeciciDurumu {
    fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
        self.state.clone()
    }
}

impl Focusable for RenkSeciciDurumu {
...
/// Bir renk seçici öğe.
#[derive(IntoElement)]
pub struct RenkSecici {
    id: ElementId,
    style: StyleRefinement,
    state: Entity<RenkSeciciDurumu>,
...
impl RenkSecici {
    /// Yeni bir renk seçici öğe ile verilen [`RenkSeciciDurumu`] oluşturur.
    pub fn new(state: &Entity<RenkSeciciDurumu>) -> Self {
        Self {
    ...
    /// featured renkler olmak için gösterilen içinde renk seçici ayarlar.
    ///
    /// Kullanıcının hızlıca seçebileceği ayarlı renkleri göstermek için kullanılır,
    /// örneğin sağlanan kullanıcı's son kullanılır renkler.
    pub fn featured_colors(mut self, colors: Vec<Hsla>) -> Self {
        self.featured_colors = Some(colors);
        self
    }

    /// simge için renk seçici düğme ayarlar.
    ///
    /// Bu ayarlanırsa renk seçici düğmesi simge gösterir.
    /// Aksi halde geçerli değerin renginde bir kare gösterir.
    pub fn icon(mut self, icon: impl Into<Simge>) -> Self {
        self.icon = Some(icon.into());
        self
    }

    /// etiket olmak için gösterilen above renk seçici ayarlar.
    ///
    /// Varsayılan `None` değeridir.
    pub fn label(mut self, label: impl Into<SharedString>) -> Self {
        self.label = Some(label.into());
        self
    }

    /// sabitleyici köşe renk seçici ayarlar.
    ///
    /// Varsayılan `Anchor::TopLeft` değeridir.
        ...
        v_flex()
            ...
            .child(
                SekmeCubugu::new("mode")
                    ...
                    .child(Sekme::new().flex_1().label(t!("RenkSecici.Palette")))
                    .child(Sekme::new().flex_1().label(t!("RenkSecici.HSLA"))),
            )
            ...
            .when_some(hovered_color, |this, hovered_color| {
                this.child(Ayirici::horizontal()).child(
                    h_flex()
                        .gap_2()
                        .items_center()
                        .child(
                ...
                h_flex()
                    ...
                    .child(
                        div()
                            ...
                            .child(
                                Kaydirici::new(&hsla_sliders.hue)
                                    .flex_1()
                ...
                h_flex()
                    ...
                    .child(
                        div()
                            ...
                            .child(
                                Kaydirici::new(&hsla_sliders.saturation)
                                    .flex_1()
                                    .bg(cx.theme().transparent),
                            ),
                    )
                ...
                h_flex()
                    ...
                    .child(
                        div()
                            ...
                            .child(
                                Kaydirici::new(&hsla_sliders.lightness)
                                    .flex_1()
                                    .bg(cx.theme().transparent),
                            ),
                    )
                ...
                h_flex()
                    ...
                    .child(
                        div()
                            ...
                            .child(
                                Kaydirici::new(&hsla_sliders.alpha)
                                    .flex_1()
                                    .bg(cx.theme().transparent),
                            ),
                    )
...
impl Focusable for RenkSecici {
    fn focus_handle(&self, cx: &App) -> FocusHandle {
        self.state.read(cx).focus_handle.clone()
    }
}

impl Styled for RenkSecici {
    fn style(&mut self) -> &mut StyleRefinement {
        &mut self.style
    }
}

impl RenderOnce for RenkSecici {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div()
            ...
            .on_action(window.listener_for(&self.state, RenkSeciciDurumu::on_confirm))
            .child(
                AcilirKatman::new("popover")
                    .open(state.open)
                    .w_72()
                    .on_open_change(
                    ...
                    div()
                        ...
                        .when_some(self.tooltip, |this, tooltip| {
                            this.managed_tooltip(move |window, cx| {
                                AracIpucu::new(tooltip.clone()).build(window, cx)
                            })
                        }),
                )
            })
