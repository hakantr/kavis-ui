...
impl From<Keystroke> for KlavyeTusu {
...
impl KlavyeTusu {
    /// Verilen [`Keystroke`] ile yeni bir KlavyeTusu öğesi oluşturur.
    ...
    /// Kısayolun görünümünü ayarlar, varsayılan `true` değeridir.
    pub fn appearance(mut self, appearance: bool) -> Self {
        self.appearance = appearance;
        self
    }

    /// Kısayol için çerçeve stilini kullanır, varsayılan `false` değeridir.
    pub fn outline(mut self) -> Self {
        self.outline = true;
        self
    }

    /// Verilen eylem ve bağlam için ilk kısayolu döndürür.
    ...
    /// Verilen eylem ve odak tutamacı için ilk kısayolu döndürür.
    ...
    /// KeyStroke üzerinden platforma özel kısayol metnini döndürür
    ///
    /// macOS: https://destek.apple.com/en-us/HT201236
...
mod tests {
    ...
    fn test_format() {
        ...
        if cfg!(target_os = "macos") {
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("cmd-a").unwrap()),
                "⌘A"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("cmd--").unwrap()),
                "⌘-"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("cmd-+").unwrap()),
            ...
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("cmd-enter").unwrap()),
                "⌘⏎"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("secondary-f12").unwrap()),
                "⌘F12"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("shift-pagedown").unwrap()),
                "⇧Page Down"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("shift-pageup").unwrap()),
                "⇧Page Up"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("shift-space").unwrap()),
                "⇧Space"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("cmd-ctrl-a").unwrap()),
                "⌃⌘A"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("cmd-alt-backspace").unwrap()),
                "⌥⌘⌫"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("shift-delete").unwrap()),
                "⇧⌫"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("cmd-ctrl-shift-a").unwrap()),
                "⌃⇧⌘A"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("cmd-ctrl-shift-alt-a").unwrap()),
                "⌃⌥⇧⌘A"
            );
        } else {
            assert_eq!(KlavyeTusu::format(&Keystroke::parse("a").unwrap()), "A");
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("ctrl-a").unwrap()),
                "Ctrl+A"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("shift-space").unwrap()),
            ...
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("ctrl-alt-a").unwrap()),
                "Ctrl+Alt+A"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("ctrl-alt-shift-a").unwrap()),
                "Ctrl+Alt+Shift+A"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("ctrl-alt-shift-win-a").unwrap()),
                "Ctrl+Alt+Shift+Win+A"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("ctrl-shift-backspace").unwrap()),
                "Ctrl+Shift+Backspace"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("alt-delete").unwrap()),
                "Alt+Delete"
            );
            assert_eq!(
                KlavyeTusu::format(&Keystroke::parse("alt-tab").unwrap()),
                "Alt+Tab"
            );
        }
    }
