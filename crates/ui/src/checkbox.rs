...
impl OnayKutusu {
    /// Yeni bir OnayKutusu ile verilen id oluşturur.
    ...
    /// araç ipucu metin için onay kutusu ayarlar.
    pub fn tooltip(mut self, tooltip: impl Into<SharedString>) -> Self {
        self.tooltip.text = Some((tooltip.into(), None));
        self
    }

    /// etiket için onay kutusu ayarlar.
    pub fn label(mut self, label: impl Into<Text>) -> Self {
        self.label = Some(label.into());
        self
    }

    /// Onay kutusunun işaretli durumunu ayarlar.
    pub fn checked(mut self, checked: bool) -> Self {
        self.checked = checked;
        self
    }

    /// tıklama işleyici için onay kutusu ayarlar.
    ///
    /// `&bool` parametresi tıklama sonrası yeni işaretli durumunu belirtir.
    pub fn on_click(mut self, handler: impl Fn(&bool, &mut Window, &mut App) + 'static) -> Self {
        self.on_click = Some(Rc::new(handler));
        self
    }

    /// Onay kutusunun sekme durağını ayarlar. Varsayılan true değeridir.
    pub fn tab_stop(mut self, tab_stop: bool) -> Self {
        self.tab_stop = tab_stop;
        self
    }

    /// Onay kutusunun sekme indeksini ayarlar. Varsayılan 0dır.
...
impl ParentElement for OnayKutusu {
...
pub(crate) fn checkbox_check_icon(
    ...
    svg()
        ...
        .map(|this| match checked {
            true => this.path(SimgeAdi::Check.path()),
            _ => this,
        })
        .map(|this| {
...
impl RenderOnce for OnayKutusu {
