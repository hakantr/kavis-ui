    ...
    V: Copy + PartialOrd + Num + ToPrimitive + Sealed + 'static,
{
    ...
    /// Her veri öğesini band ekseni değerine eşler (kategorik/sıralı eksen).
    pub fn band(mut self, band: impl Fn(&T) -> B + 'static) -> Self {
        self.band = Some(Rc::new(band));
        self
    }

    /// Her veri öğesini değer ekseni boyunca sayısal değerine eşler.
    pub fn value(mut self, value: impl Fn(&T) -> V + 'static) -> Self {
        self.value = Some(Rc::new(value));
        self
    }

    /// Her veri noktası için olduğu gibi kullanılacak dolguyu ayarlar.
    ///
    /// kapanış receives:
    ///
    /// 1. datum,
    /// 2. **çubuk's sınırlar** içinde piksel boşluk, expressed göreli için
    /// grafik's origin (i.e. çubuk's çizilmiş rectangle içinde grafik),
    /// 3. **grafik's sınırlar** içinde piksel boşluk ile origin `(0, 0)` ve boyut
    /// equal için tam grafik extent, ve
    /// 4. Çubuğun [`BarAlignment`] değeri (çağıran taraf yönelime göre dallanabilir,
    /// e.g. flip bir gradyan açı).
    ///
    /// Her iki dikdörtgen aynı koordinat sistemini paylaşır; çağıranlar
    /// implement arbitrary grafik-aware backgrounds — çubuk-yerel gradients,
    /// grafik-wide gradients, patterns, sampled colormaps, etc. — olmadan herhangi bir
    /// kütüphaneden yardım almadan işlem yapabilir.
    ///
    /// Accepts herhangi bir tip convertible için [`arka plan`]. ayar bu temizler herhangi bir
    /// previously ayarlar [`BarChart::fill_gradient`].
    pub fn fill<Bg>(
        mut self,
    ...
    /// bir başına-datum auto-oriented doğrusal gradyan fill ayarlar.
    ///
    /// kapanış receives datum, grafik's tam veri aralık
    /// (`chart_range`, tüm veri değerlerinden türetilir) ve bir `chart_to_bar`
    /// yeniden eşleme yardımcısı grafik değeri koordinatını çubuğa yerel
    /// Gradyan konumu (`0.0` çubuğun tabanı, `1.0` çubuğun ucu).
    ///
    /// Her çubuk için ayrı gradyanlarda çubuk yerel konumlarını doğrudan kullanır (her çubuk
    /// looks aynı regardless onun değer):
    ///
    /// ```ignore
    /// .fill_gradient(|_, _, _| [
    ...
    /// ```
    ///
    /// Ya da durakları grafiğe göreli değerlerde konumlandırmak için `chart_to_bar` kullanın; böylece
    /// Her çubuk, grafik genelindeki gradyanın ilgili dilimini gösterir.
    /// kendi `[base, değer]` aralığı:
    ///
    /// ```ignore
    ...
    /// ```
    ///
    /// `[0, 1]` dışında dönen durak konumları çubuğa kırpılır;
    /// kütüphane kırpma noktalarındaki renkleri enterpole eder; böylece çubuk üzerindeki gradyan
    /// still matches grafik-wide bir.
    ///
    /// Gradyan açısı [`BarAlignment`] değerinden türetilir; böylece stop-0
    /// taban, stop-1 ise uç nokta olur. Bu ayar daha önce yapılmış değerleri temizler.
    /// [`BarChart::fill`].
    ...
    /// Gösterir veya gizler band-eksen satır ve etiketler.
    ///
    ...
    ///
    /// Kullanım [`Corners::all`] için uniform rounding, veya construct [`Corners`] manually
    /// Yalnızca özel köşeleri yuvarlamak için kullanılır (ör. her çubuğun yalnızca uç bitişi).
...
/// İki duraklı gradyanı çubuğa yerel `[0, 1]` aralığına kırpar ve renkleri
/// clip points böylece üzerinde-çubuk gradyan matches (possibly broader) gradyan
/// çağıran defined.
///
/// Olduğunda bir stop konum falls dışında `[0, 1]` (e.g. because `chart_to_bar`
/// döndürülen bir değer past çubuk's kenar için bir grafik-göreli gradyan),
/// gpui's renderer olurdu sınırlandırır konum ve lose gradyan effect.
/// Bu fonksiyon bunun yerine değiştirir böyle bir stop ile renk sampled boyunca
/// iki duraktan geçen çizgi üzerinde `0.0` veya `1.0` konumunu örnekleyerek korur.
/// görsel slice.
