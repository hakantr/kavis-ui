...
impl Avatar {
    ...
    /// kullanmak için image kaynak için avatar ayarlar.
    pub fn src(mut self, source: impl Into<ImageSource>) -> Self {
        self.src = Some(source.into());
        self
    }

    /// Avatar kullanıcısının adını ayarlar. `src` yoksa bu ad yer tutucu olarak kullanılır.
    ...
    /// yer tutucu simge ayarlar. Varsayılan: [`SimgeAdi::User`]
    pub fn placeholder(mut self, icon: impl Into<Simge>) -> Self {
