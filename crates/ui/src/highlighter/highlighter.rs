...
use tree_sitter::{
    InputEdit, ParseOptions, Parser, Point, Query, QueryCursor, StreamingIterator, Tree as Agac,
};

/// Olduğunda bir düğüm spans daha fazla than bu many bayt beyond requested sorgu
/// aralıksa doğrudan sorgulamak yerine alt öğelerine özyinelemeli ineriz.
const LARGE_NODE_THRESHOLD: usize = 8 * 1024;

/// Bir syntax vurgulayıcı olan destekler artımlı ayrıştırma, multiline metin,
/// ve caching vurgulama results.
#[allow(unused)]
pub struct SozdizimiVurgulayici {
    ...
    /// son ayrıştırılmış kaynak metin.
    text: Rope,
    parser: Parser,
...
/// Enjeksiyon katmanlarını hesaplamak için gereken veri; arka plan threadinde kullanılır.
...
/// Vurgulama öğesi; aralık ağaç içindeki belirteç ofsetidir.
#[derive(Debug, Default, Clone)]
struct HighlightItem {
    /// bayt aralık vurgulama içinde metin.
    range: Range<usize>,
    /// vurgulama ad, gibi `fonksiyon`, `metin`, `comment`, etc.
    name: SharedString,
}
...
impl SozdizimiVurgulayici {
    ...
    fn build_combined_injections_query(lang: &str) -> Result<Self> {
        ...
        for inj_language in config.injection_languages.iter() {
            if let Some(inj_config) = DilKaydi::singleton().language(&inj_language) {
                match Query::new(&inj_config.language, &inj_config.highlights) {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "{:?} için enjeksiyon sorgusu oluşturulamadı: {:?}",
                            inj_config.name,
                            e
                        );
                    }
    ...
    /// ayrıştırılmış ağaç (ise kullanılabilir) döndürür.
    pub fn tree(&self) -> Option<&Agac> {
        self.tree.as_ref()
    }

    /// dil ad için bu vurgulayıcı döndürür.
    pub fn language(&self) -> &SharedString {
        &self.language
    }

    /// bir referans için geçerli metin. döndürür.
    pub fn text(&self) -> &Rope {
        &self.text
    }

    /// Verilen metni vurgular ve bayt aralıklarından vurgulama yakalamalarına eşleme döndürür.
    ///
    /// `edit` ile artımlı ayrıştırma kullanarak vurgulayıcı durumunu verimli şekilde günceller.
    /// `timeout` Some olduğunda ayrıştırma verilen süreyi aşarsa durdurulur.
    /// ve `false` döndürür. Zaman aşımında eski ağaç korunur; böylece vurgulama
    /// Eski veriyle çalışmaya devam eder, ancak `self.metin` güncellenir; böylece
    /// Çağıran, geçerli metni arka plan ayrıştırıcısına gönderebilir.
    /// `timeout` None olduğunda ayrıştırma tamamlanana kadar çalışır ve her zaman `true` döndürür.
    pub fn update(
        &mut self,
    ...
    /// Yeni ayrıştırılmış ana ağaç için enjeksiyon katmanlarını hesaplar.
    /// Bu yan etkisiz saf bir hesaplamadır ve arka plan threadinde
    /// arka plan threadinde çalıştırılabilir.
    ...
    ///
    /// `injection_layers` ayrıca arka planda önceden hesaplanmış olmalıdır;
    /// ana threadin bloklanmasını önlemek için [`compute_injection_layers`] kullanılır.
    ...
    /// Vurgulama için Agac içindeki görünür aralıklarla eşleşen düğümler.
    ...
    /// syntax vurgulama stiller için aralık metin döndürür.
    ///
    /// `aralık` argümanı, vurgulanacak metindeki bayt aralığıdır.
    ///
    /// Her demetin şunları içerdiği bir demet vektörü döndürür:
    /// - Bir bayt aralık göreli için metin
    /// - corresponding vurgulama stil için olan aralık
    ///
    /// # Örnek
    ///
    /// ```no_run
    /// use kavis_ui::highlighter::{VurguTemasi, SozdizimiVurgulayici};
    /// use ropey::Rope;
    ///
    /// let code = "fn main() {\n    println!(\"Hello\");\n}";
    /// let rope = Rope::from_str(code);
    /// let mut highlighter = SozdizimiVurgulayici::new("rust");
...
/// To birleştirir intersection aralıklar, let subsequent aralık cover
/// Önceki çakışan aralık ve önceki aralığı bölme.
///
/// Den:
///
/// AA
...
/// Ağacı dolaşır ve sorguya uygun düğümleri toplar; gereksiz alt ağaçları atlar.
/// olan fall entirely dışında bayt aralık. Nodes much daha büyük than
/// Sorgu aralığına girilerek `QueryCursor` yalnızca ilgili düğümleri ziyaret eder.
/// relevant portion ağaç.
