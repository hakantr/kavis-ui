...
use crate::{
    ...
    clipboard::Pano,
    description_list::AciklamaListesi,
    h_flex,
...
/// Denetleyiciyi başlatır ve açıp kapatma eylemini kaydeder.
...
struct EditorState {
    /// Düzenleyici için giriş durumu.
    state: Entity<InputState>,
    /// Girdi ayrıştırılırken ya da serileştirme hatası oluşursa gösterilecek hata.
    error: Option<SharedString>,
    /// Düzenleyicinin şu anda düzenlenip düzenlenmediği.
    editing: bool,
}
...
pub struct DivInspector {
    ...
    /// Herhangi bir düzenlemeden önceki başlangıç stili
    initial_style: StyleRefinement,
    /// Başlangıç stilinin Rust koduna dönüştürülemeyen bölümü
...
fn rust_to_style(mut style: StyleRefinement, source: &str) -> (StyleRefinement, Vec<Tani>) {
                    ...
                    v_flex()
                        ...
                        .child(
                            h_flex()
                                ...
                                .gap_x_2()
                                .child("Rust Styles")
                                .child(Dugme::new("rust-reset").label("Reset").small().on_click(
                        ...
                        .child(
                            v_flex()
                                ...
                                .when_some(self.rust_state.error.clone(), |this, err| {
                                    this.child(Uyari::error("rust-error", err).text_xs())
                                }),
                        ),
                )
                .child(
                    v_flex()
                        ...
                        .child(
                            h_flex()
                                ...
                                .child(Dugme::new("json-reset").label("Reset").small().on_click(
                                    cx.listener(|this, _, window, cx| {
                                        this.reset_style(window, cx);
                                    }),
                                )),
                        ...
                        .child(
                            v_flex()
                                ...
                                .when_some(self.json_state.error.clone(), |this, err| {
                                    this.child(Uyari::error("json-error", err).text_xs())
                                }),
                        ),
                )
            },
                ...
                .when_some(source_location, |this, source_location| {
                    this.child(
                        h_flex()
                            ...
                            .child(Pano::new("copy-source-location").value(source_location)),
                    )
                })
