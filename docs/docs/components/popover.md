...
use kavis_ui::{button::Button, popover::Popover};
...
```rust
use kavis_ui::{DismissEvent, popover::Popover};
...
Like the others components in Kavis UI, the `appearance(false)` method can be used to disable the default styling of the popover, allowing you to fully customize its appearance.
...
[Selectable]: https://docs.rs/kavis-ui/latest/kavis_ui/trait.Selectable.html
