...
```rust
use kavis_ui::input::{InputState, Input};
```
...
Kavis UI's `InputState` supports a code editor mode with syntax highlighting, line numbers, and search functionality.
...
Enable `kavis-ui/tree-sitter-languages` to include all built-in Tree-sitter grammars, or enable only the grammars you need, such as `kavis-ui/tree-sitter-markdown` or `kavis-ui/tree-sitter-rust`, to reduce downstream bundle size.
