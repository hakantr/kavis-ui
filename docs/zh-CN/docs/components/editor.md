...
Kavis UI 的 `InputState` 支持代码编辑器模式，可提供语法高亮、行号和搜索功能。
...
启用 `kavis-ui/tree-sitter-languages` 可包含所有内置 Tree-sitter 语法；如果只需要少量语言，可以只启用 `kavis-ui/tree-sitter-markdown` 或 `kavis-ui/tree-sitter-rust` 等单语言 feature，以减少下游应用体积。
