...
Kavis UI 中的 [IconName] 和 [Icon] 提供了一套可直接在 GPUI 应用中使用的图标接口。

但为了尽量减小应用体积，`kavis-ui` 默认 **不会内置任何图标资源**。

因此仓库把图标资源拆分到了独立的 [kavis-ui-assets] crate 中。这样你可以自行决定：
...
[kavis-ui-assets] 提供了一个默认的资源实现，包含 `assets/icons` 目录下的全部图标文件。
...
- [Lucide Icons](https://lucide.dev/) - Kavis UI 的图标集主要基于 Lucide 开源图标库
